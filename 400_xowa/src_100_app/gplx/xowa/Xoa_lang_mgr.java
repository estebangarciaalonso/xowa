...
public class Xoa_lang_mgr implements GfoInvkAble {
	...
	public Cfg_nde_root Groups() {return groups;} Cfg_nde_root groups = new Cfg_nde_root().Root_(new Xoac_lang_grp(ByteAry_.Empty), Xoac_lang_grp.Make_grp, ByteAry_.Ary_empty);
	...
	public void Local_set_bulk(byte[] src) {	// NOTE: setting local lang names/grps on app level; may need to move to user level or wiki level (for groups) later
		...
		byte[] code = ByteAry_.Empty, name = ByteAry_.Empty;
