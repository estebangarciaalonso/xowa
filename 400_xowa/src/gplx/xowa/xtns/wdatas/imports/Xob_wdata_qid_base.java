...
public abstract class Xob_wdata_qid_base extends Xob_itm_dump_base implements Xobd_wkr, GfoInvkAble {
	...
	public void Wkr_run(Xodb_page page) {
		...
		for (int i = 0; i < len; i++) {
			...
			if (ByteAry_.Eq(xwiki_key, Xwiki_key_en)) data_core = data;
		}
		...
		for (int i = 0; i < len; i++) {
			...
			if (core_ns_is_main) {
				...
				if (colon_pos == ByteAry_.NotFound || colon_pos == data_ttl_len - 1) {
					...
					actl_ttl = ByteAry_.Mid(data_ttl_bry, colon_pos + 1, data_ttl_len);
			}
			this.Qid_add(data.Xwiki_key(), core_ns, actl_ttl, qid);
		}
	}	ListAdp links = ListAdp_.new_(); Xob_bz2_file tmp_xwiki = new Xob_bz2_file(); ByteAryBfr tmp_bfr = ByteAryBfr.reset_(255);
	public void Wkr_end() {
		this.Qid_end();
	}	static final byte[] Xwiki_key_en = ByteAry_.new_ascii_("enwiki");
