...
public class Xodb_save_mgr_txt implements Xodb_save_mgr {
	...
	public void Data_create(Xoa_ttl ttl, byte[] text) {
		...
		if (ByteAry_.Len_gt_0(page_rdr_bry)) page_rdr.Parse(page_rdr_bry, page_rdr_bry.length, page_rdr_url);
	...
	public void Data_rename(Xoa_page page, byte[] new_ttl)	{
		if (wiki.Wiki_tid() != Xow_wiki_type_.Tid_home) {
	...
	private void Data_update_under(Xoa_page page, byte[] text, byte[] new_ttl) {
		...
		ByteAryBfr tmp = wiki.Utl_bry_bfr_mkr().Get_b512();
		Xodb_page_.Txt_ttl_save(tmp, db_page);
	...
	static final String GRP_KEY = "xowa.wiki.db.save";
}
