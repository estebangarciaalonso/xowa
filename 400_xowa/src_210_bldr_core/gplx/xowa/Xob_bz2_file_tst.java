...
class Xob_bz2_file_fxt {
	...
	public void Build_wmf_src_dir(String alias, String date, String expd) {
		byte[] actl = Xob_dump_file_.Bld_dump_dir_url(ByteAry_.new_ascii_(Xob_dump_file_.Server_your_org), ByteAry_.new_ascii_(alias), ByteAry_.new_ascii_(date));
		Tfds.Eq(expd, String_.new_ascii_(actl));
	}
	public void Build_wmf_src_name(String alias, String date, String dump_type, String expd) {
		byte[] actl = Xob_dump_file_.Bld_dump_file_name(ByteAry_.new_ascii_(alias), ByteAry_.new_ascii_(date), ByteAry_.new_ascii_(dump_type), Xob_dump_file_.Ext_xml_bz2);
		Tfds.Eq(expd, String_.new_ascii_(actl));
	}
	public void Build_alias_by_lang_tid(String lang_key, byte wiki_tid, String expd) {
		Xob_bz2_file.Build_alias_by_lang_tid(tmp_bfr, ByteAry_.new_ascii_(lang_key), wiki_tid_ref.Val_(wiki_tid));
		Tfds.Eq_bry(ByteAry_.new_utf8_(expd), tmp_bfr.XtoAryAndClear());
	}	static final ByteRef wiki_tid_ref = ByteRef.zero_();
}
