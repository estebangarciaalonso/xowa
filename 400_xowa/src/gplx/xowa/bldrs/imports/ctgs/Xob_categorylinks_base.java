...
public abstract class Xob_categorylinks_base extends Xob_sql_dump_base implements Sql_file_parser_cmd {
	...
	public void Exec(byte[] src, byte[] fld_key, int fld_idx, int fld_bgn, int fld_end, ByteAryBfr file_bfr, Sql_file_parser_data data) {
		if		(ByteAry_.Eq(fld_key, Fld_cl_from))			cur_id = ByteAry_.X_to_int_or(src, fld_bgn, fld_end, -1);
		...
		else if (ByteAry_.Eq(fld_key, Fld_cl_type)) {
			...
			if (cur_collation_is_uca) {
				...
				cur_sortkey = uca_bfr.Bry_len() == 0 ? Sortkey_space : uca_bfr.XtoAryAndClear();
			}
	...
	DateAdp_parser date_parser = DateAdp_parser.new_(); Sql_file_parser sql_parser; Uca_trie trie; ByteAryBfr uca_bfr = ByteAryBfr.reset_(255);		
	private static final byte[] Collation_uca = ByteAry_.new_utf8_("uca"), Sortkey_space = new byte[] {Byte_ascii.Space};
}
