...
public class Xof_offline_ogg_tst {
	...
	@Test   public void Audio() {
		...
		fxt.Exec_get(Xof_fsdb_arg_exec_get.new_().Init_orig("A.ogg").Rslt_reg_missing_reg().Rslt_qry_mock_().Rslt_bin_noop_().Rslt_cnv_n_());
		Xof_fsdb_itm itm = fxt.Test_regy_pass("A.ogg");
		fxt.Test_itm_ext(itm, Xof_ext_.Id_oga);// chk that file is now oga
	}
}
