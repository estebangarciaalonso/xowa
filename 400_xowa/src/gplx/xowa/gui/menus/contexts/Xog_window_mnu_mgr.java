...
public class Xog_window_mnu_mgr implements GfoInvkAble {
	...
	public Xog_window_mnu_mgr(Xoa_app app, Xog_menu_mgr menu_mgr) {
		this.app = app;
		Get_or_new("main_win").Source_default_(Xog_mnu_defaults.Main_win_mnu);	// NOTE: set default here (fires before cfg)
	}	private Xoa_app app;
	public void Init_by_kit() {
		Xog_mnu_grp mnu = Get_or_new("main_win");	// NOTE: build menu now; NOTE: do not set default here, or else will override user setting
		mnu.Source_exec(app.Gfs_mgr());
	}
	public Xog_mnu_grp Get_or_new(String key) {			
		Xog_mnu_grp rv = (Xog_mnu_grp)hash.Fetch(key);
		if (rv == null) {
			rv = new Xog_mnu_grp(app, false);
			hash.Add(key, rv);
		}
		return rv;
	}	
