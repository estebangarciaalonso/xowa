...
public class Map_dd2dms_func extends Pf_func_base {
	...
	@Override public void Func_evaluate(Xop_ctx ctx, byte[] src, Xot_invk caller, Xot_invk self, ByteAryBfr bfr) {
		...
		byte[] plus = ByteAry_.Empty, minus = ByteAry_.Empty;
		int prec = 4;
		for (int i = 0; i < args_len; i++) {
			...
			if (key_tid_obj != null) {
				...
				switch (((ByteVal)key_tid_obj).Val()) {
					...
					case Key_tid_precision:	prec = ByteAry_.X_to_int_or(val, prec); break;
				}
			}
		}
		tmp_bfr.Mkr_rls();
		Map_math map_math = Map_math._;
		if (map_math.Ctor(coord, prec, ByteAry_.Empty, 2))
