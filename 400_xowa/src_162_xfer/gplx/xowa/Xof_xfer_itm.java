...
public class Xof_xfer_itm {
	...
	public int			Lnki_seek() {return lnki_seek;} private int lnki_seek = Xop_lnki_tkn.Thumbtime_null;
	...
	public Xof_xfer_itm Atrs_by_lnki(byte lnki_type, int w, int h, double upright, int seek) {this.lnki_type = lnki_type; this.lnki_w = w; this.lnki_h = h; this.lnki_upright = upright; this.lnki_seek = seek; lnki_thumbable = Xof_xfer_itm.Lnki_thumbable_calc(lnki_type, lnki_w, lnki_h); return this;}
	...
	public boolean Atrs_calc_for_html(boolean caller_is_file_page) {
		...
		if (ext.Id_is_media() && html_w < 1)		// if media and no width, set to default; NOTE: must be set or else dynamic download will resize play button to small size; DATE:20121227
			html_w = Xof_img_mgr.Thumb_w_ogv_const;	
		if (lnki_thumbable) {				// file is thumb
			if (ext.Id_is_video()) {		// video is a special case; src is thumb_w but html_w / html_h is based on calc
				html_orig_src = Trg_html(Xof_repo_itm.Mode_orig, Xof_url_.Null_size_deprecated);
				...
				Xof_meta_thumb thumb = meta_itm.Thumbs_get_vid(lnki_seek);
				if (thumb != null) {
			...
			if		(ext.Id_is_audio()) return true;	// if audio, return true; SEE:NOTE_2
				...
				Xof_meta_thumb thumb = meta_itm.Thumbs_get_vid(lnki_seek);	// get thumb at lnki_seek; NOTE: in most cases this will just be the 1st thumb; note that orig video files don't have an official thumb
				if (thumb != null) {
	...
	byte[] Trg_html(byte mode_id, int width)		{return Xof_url_bldr.Temp.Set_trg_html_(mode_id, trg_repo, ttl, md5, ext, width, lnki_seek).Xto_bry();}
	public Io_url Trg_file(byte mode_id, int width) {return Xof_url_bldr.Temp.Set_trg_file_(mode_id, trg_repo, ttl, md5, ext, width, lnki_seek).Xto_url();}
	public Xof_xfer_itm Clear() {
		lnki_type = Byte_.MaxValue_127; lnki_w = Int_.Neg1; lnki_h = Int_.Neg1; lnki_upright = Int_.Neg1; lnki_seek = Int_.Neg1; lnki_thumbable = false;
	...
	public Xof_xfer_itm Clone() {
		...
		rv.lnki_type = lnki_type; rv.lnki_w = lnki_w; rv.lnki_h = lnki_h; rv.lnki_upright = lnki_upright; rv.lnki_seek = lnki_seek; rv.lnki_thumbable = lnki_thumbable;
