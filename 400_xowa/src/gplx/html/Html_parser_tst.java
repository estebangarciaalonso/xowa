...
public class Html_parser_tst {
	Xoh_parser_fxt fxt = new Xoh_parser_fxt();
...
class Xoh_parser_fxt {
	...
	public Xoh_parser_fxt Test_parse_find(String raw_str, String find_key, String find_val, String... expd) {
		...
		Html_nde[] actl_ndes = parser.Parse_as_ary(raw, 0, raw.length, ByteAry_.new_ascii_(find_key), ByteAry_.new_ascii_(find_val));
		String[] actl = Xto_ids(raw, actl_ndes);
		Tfds.Eq_ary_str(expd, actl);
		return this;
	}
	String[] Xto_ids(byte[] src, Html_nde[] ary) {
		int len = ary.length;
