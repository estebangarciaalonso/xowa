...
public class Pf_intl_int_tst {
	...
	@Test  public void Month_en()				{fxt.Test_parse_tmpl_str_test("{{int:january}}"								, "{{test}}"	, "January");}
	@Test  public void Month_en_upper()			{fxt.Test_parse_tmpl_str_test("{{int:JANUARY}}"								, "{{test}}"	, "January");}
	@Test  public void Unknown()				{fxt.Test_parse_tmpl_str_test("{{int:unknown_msg}}"							, "{{test}}"	, "<unknown_msg>");}
	@Test  public void Fmt()					{fxt.Test_parse_tmpl_str_test("{{int:pfunc_expr_unrecognised_word|1a}}"		, "{{test}}"	, "Expression error: Unrecognised word \"1a\"");}
	@Test  public void Lang_specified()			{fxt.Test_parse_tmpl_str_test("{{int:January/en}}"							, "{{test}}"	, "January");}
	@Test  public void Lang_specified_by_page() {
		...
		fxt.Test_parse_tmpl_str_test("{{int:Lang}}", "{{test}}"	, "fr");
	...
	@Test  public void Unknown_defaults_to_english() {	// PURPOSE: if key does not exist in non-english language, use English; EX: la.wikipedia.org/wiki/Fasciculus:HannibalFrescoCapitolinec1510.jpg; DATE:2013-09-10
		...
		fxt.Ctx().Page().Lang_(fr_lang); 
		fxt.Test_parse_tmpl_str_test("{{int:en_only_key}}", "{{test}}"	, "en_only_val");
		fxt.Ctx().Page().Lang_(en_lang);		// NOTE: must reset back to en_lang, else rest of tests will look up under fr
	}
	@Test  public void Tmpl_txt() {
		...
		fxt.Test_parse_tmpl_str_test("{{int:tst_msg}}"							, "{{test}}"	, "1");
	}
	@Test  public void Mediawiki_precedes_gfs() {
		...
		fxt.Test_parse_tmpl_str_test("{{int:precedes}}"							, "{{test}}"	, "precedes_from_mw");
	}
	@Test  public void Convert_php() {
		fxt.Data_create("MediaWiki:tst_msg_2", "a\\\\b\\$c$1e");
		fxt.Test_parse_tmpl_str_test("{{int:tst_msg_2|d}}"						, "{{test}}"	, "a\\b$cde");
	}
	@Test  public void Convert_php_tilde() {	// PURPOSE: tildes should be escaped, else will fail inside ByteAryBfrFmtr; DATE:2013-11-11
		...
		fxt.Test_parse_tmpl_str_test("{{int:tst_msg_3|a}}"						, "{{test}}"	, "a~u");
	}
	@Test  public void Err_fmt_failed() {
		...
		fxt.Test_parse_tmpl_str_test("{{int:tst_msg}}"							, "{{test}}"	, "a$1b");
	}
}
