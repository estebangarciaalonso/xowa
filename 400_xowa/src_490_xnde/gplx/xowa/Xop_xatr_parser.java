...
public class Xop_xatr_parser {	// REF.MW:Sanitizer.php|decodeTagAttributes;MW_ATTRIBS_REGEX
	...
	int Xnde_find_gt(Gfo_msg_log log_mgr, byte[] src, int lt_pos, int end) {
		...
		if (match_pos == String_.NotFound) {log_mgr.Add_str_warn_key_none(Msg_mgr, "invalid lt", src, lt_pos); return String_.NotFound;}
		...
		for (int i = match_pos; i < end; i++) {
			...
			switch (b) {
				...
				case Byte_ascii.Slash:
					if (slash_found)		{log_mgr.Add_str_warn_key_none(Msg_mgr, "multiple slashes not allowed", src, i); return String_.NotFound;}	// only allow one slash
...
//								break;
		...
		if (atr_bgn != -1) {		// atr_bgn will be -1 if atrs ends on quoted (EX:"a='b'"); otherwise, there is one in buffer that needs to be processed
			val_end = end;
