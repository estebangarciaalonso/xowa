...
public class Xof_qry_wkr_wmf_api implements Xof_qry_wkr {
	...
	private Xoapi_orig_base orig_api;
	private Xoapi_orig_rslts api_rv = new Xoapi_orig_rslts();
	public Xof_qry_wkr_wmf_api(Xow_wiki wiki, Xoapi_orig_base orig_api) {this.wiki = wiki; this.orig_api = orig_api;}
	public byte Tid() {return Xof_qry_wkr_.Tid_wmf_api;}
	public boolean Qry_file(Xof_fsdb_itm itm) {
		byte[] itm_ttl = itm.Lnki_ttl();
		boolean found = orig_api.Api_query_size(api_rv, wiki, itm_ttl, itm.Lnki_w(), itm.Lnki_h());
		if (!found) return false;	// ttl not found by api; return
		itm.Orig_wiki_(api_rv.Orig_wiki());
		if (!Bry_.Eq(itm_ttl, api_rv.Orig_page()))	// ttl is different; must be redirect
			itm.Orig_redirect_(api_rv.Orig_page());
