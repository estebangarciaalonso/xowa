...
import gplx.dbs.*;
public class Fsdb_db_sys_itm implements RlsAble {
	...
	public Fsdb_db_sys_itm Init(Io_url dir, String wiki_domain) {
		Io_url url = dir.GenSubFil(wiki_domain + "#main#file.sys#0000.sqlite3");
		if (Io_mgr._.ExistsFil(url))
			Init_load(dir, url, wiki_domain);
		else
			Init_make(dir, url, wiki_domain);
		return this;
	}
	public void Thm_insert(Fsdb_xtn_thm_itm rv, byte[] dir, byte[] fil, int ext_id, int w, int h, int thumbtime, DateAdp modified, String hash, long bin_len, gplx.ios.Io_stream_rdr bin_rdr) {
		...
		bin_mgr.Insert(bin_db_id, thm_id, Fsdb_bin_tbl.Owner_tid_thm, bin_len, bin_rdr);
	}
	...
	private void Init_load(Io_url dir, Io_url boot_url, String wiki_domain) {
		...
		bin_mgr = Fsdb_db_bin_mgr.load_(boot_provider, dir, wiki_domain);
	}
	private void Init_make(Io_url dir, Io_url boot_url, String wiki_domain) {
		Db_connect connect = Db_connect_sqlite.make_(boot_url);
		boot_provider = Db_provider_.new_(connect);
		Fsdb_cfg_tbl.Create_table(boot_provider);
		atr_mgr = Fsdb_db_atr_mgr.make_(boot_provider, dir, wiki_domain);
		bin_mgr = Fsdb_db_bin_mgr.make_(boot_provider, dir, wiki_domain);
		this.Commit();
	}
}
