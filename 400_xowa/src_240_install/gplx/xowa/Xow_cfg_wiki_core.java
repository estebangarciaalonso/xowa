		...
		while (true) {
			...
			switch (b) {
				case Byte_ascii.Pipe:
					switch (fld_idx) {
						case 0:		cur_id = ByteAry_.XtoIntByPos(src, fld_bgn, pos, Int_.MinValue);					if (cur_id == Int_.MinValue)		throw Err_mgr._.fmt_(GRP_KEY, "fail.load.id", "failed to load id: ~{0}", String_.new_utf8_(src, fld_bgn, pos)); break;
						case 1:		cur_case_match = ByteAry_.Xto_byte_by_int(src, fld_bgn, pos, Byte_.MaxValue_127);	if (cur_id == Byte_.MaxValue_127)	throw Err_mgr._.fmt_(GRP_KEY, "fail.load.case_match", "failed to load match: ~{0}", String_.new_utf8_(src, fld_bgn, pos)); break;
