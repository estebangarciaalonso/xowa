...
public class Xows_page_allpages_tst {
	Xows_page_allpages_fxt fxt = new Xows_page_allpages_fxt();
	@Before public void init() {fxt.Clear();}
	@Test  public void Build_data() {
		Xow_hive_mgr_fxt.Ttls_create_rng(fxt.Wiki(), 5, 7);
		fxt.Clear().Init_arg_("from", "B1").Expd_prv_("A3").Expd_nxt_("B6").Expd_ttls_("B1", "B2", "B3", "B4", "B5").Test_build_data();	// single file
		fxt.Clear().Init_arg_("from", "A6").Expd_prv_("A1").Expd_nxt_("B4").Expd_ttls_("A6", "B0", "B1", "B2", "B3").Test_build_data();	// overflow rhs
		fxt.Clear().Init_arg_("from", "E4").Expd_prv_("D6").Expd_nxt_("E6").Expd_ttls_("E4", "E5", "E6", null, null).Test_build_data();	// bounds rhs
		fxt.Clear().Init_arg_("from", "A0").Expd_prv_("A0").Expd_nxt_("A5").Expd_ttls_("A0", "A1", "A2", "A3", "A4").Test_build_data();	// bounds lhs	
		fxt.Clear().Init_arg_("from", "B0a").Expd_prv_("A3").Expd_nxt_("B6").Expd_ttls_("B1", "B2", "B3", "B4", "B5").Test_build_data();	// inexact match; B0a matches "hi" value
		fxt.Clear().Init_arg_("from", "B1").Init_arg_("hideredirects", "1").Expd_prv_("A0").Expd_nxt_("C4").Expd_ttls_("B2", "B4", "B6", "C0", "C2").Test_build_data();	// hide redirects
		fxt.Clear().Init_arg_("from", "A6").Expd_prv_("A0").Expd_nxt_("E1").Init_itms_per_page_(23).Expd_ttls_("A6", "B0", "B1", "B2", "B3", "B4", "B5", "B6", "C0", "C1", "C2", "C3", "C4", "C5", "C6", "D0", "D1", "D2", "D3", "D4", "D5", "D6", "E0").Test_build_data();	// overflow rhs x2
	}
	@Test  public void Build_html_main() {
		Xow_hive_mgr_fxt.Ttls_create_rng(fxt.Wiki(), 5, 7);
		fxt.Clear().Init_arg_("from", "B2").Init_itms_per_page_(5).Test_build_html(String_.Concat_lines_nl
	...
	@Test  public void Build_html_redirect() {
		...
		fxt.Clear().Init_arg_("from", "A2").Init_arg_("hideredirects", "1").Init_itms_per_page_(2).Test_build_html(String_.Concat_lines_nl
		(	"<table class=\"mw-allpages-table-form\">"
		,	"  <tr>"
		,	"    <td class=\"mw-allpages-nav\">"
	...
	@Test  public void Build_html_ns() {
		...
		fxt.Clear().Init_arg_("from", "A2").Init_arg_("namespace", "10").Init_itms_per_page_(2).Test_build_html(String_.Concat_lines_nl
		(	"<table class=\"mw-allpages-table-form\">"
		,	"  <tr>"
		,	"    <td class=\"mw-allpages-nav\">"
	...
	@Test   public void Misc() {
		Xow_hive_mgr_fxt.Ttls_create_rng(fxt.Wiki(), fxt.Wiki().Ns_mgr().Ns_template(), 1, 7);
		fxt.Clear().Init_arg_("from", "Template:B1").Expd_arg_("from", "B1").Expd_arg_("namespace", "10").Test_build_data();	// extract ns from ttl
		fxt.Clear().Init_arg_("from", "Template:B1").Expd_display_ttl_("All pages").Expd_address_page_("Special:AllPages").Test_special_gen();	// display ttl
	}
}
class Xows_page_allpages_fxt {
	...
	public Xows_page_allpages_fxt Init_itms_per_page_(int v) {init_itms_per_page = v; return this;} private int init_itms_per_page = 5;
	public Xows_page_allpages_fxt Expd_prv_(String v) {expd_prv = v; return this;} private String expd_prv;
	public Xows_page_allpages_fxt Expd_nxt_(String v) {expd_nxt = v; return this;} private String expd_nxt;
	public Xows_page_allpages_fxt Expd_ttls_(String... v) {expd_ttls = v; return this;} private String[] expd_ttls;
	public Xows_page_allpages_fxt Expd_display_ttl_(String v) {expd_display_ttl = v; return this;} private String expd_display_ttl;
	public Xows_page_allpages_fxt Expd_address_page_(String v) {expd_address_page = v; return this;} private String expd_address_page;

	public static String Xto_str(Xow_wiki wiki, Xodb_page v) {
		if (v == null) return null;
		Xow_ns ns = wiki.Ns_mgr().Get_by_id(v.Ns_id());
	...
	public Xows_page_allpages_fxt Test_special_gen() {
		...
		parserx.Parse(init_url, ByteAry_.new_ascii_("Special:AllPages"));
		allpages.Special_gen(init_url, wiki.Ctx().Page(), wiki, Xoa_ttl.parse_(wiki, ByteAry_.new_ascii_("Special:AllPages")));
		if (expd_display_ttl != null) Tfds.Eq(expd_display_ttl, String_.new_utf8_(wiki.Ctx().Tab().Display_ttl()));
