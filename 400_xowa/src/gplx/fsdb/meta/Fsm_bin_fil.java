...
public class Fsm_bin_fil implements RlsAble {
	...
	public Db_conn Conn() {
		if (conn == null) {
			conn =  cmd_mode == Db_cmd_mode.Tid_create
				? Db_conn_bldr.I.New(Db_conn_bldr_type, url)
				: Db_conn_bldr.I.Get(Db_conn_bldr_type, url)
				;
			bin_tbl.Conn_(conn, 1, Bool_.Y);
			if (cmd_mode == Db_cmd_mode.Tid_create)
				bin_tbl.Create_table();
		}
		return conn;
	}
	...
	public boolean Get_to_url(int id, Io_url url, byte[] bin_bfr, int bin_flush_when) {this.Conn(); return bin_tbl.Select_to_url(id, url, bin_bfr, bin_flush_when);}
