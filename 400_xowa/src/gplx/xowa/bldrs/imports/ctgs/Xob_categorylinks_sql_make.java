...
public class Xob_categorylinks_sql_make implements Io_make_cmd {
	...
	int Ctg_grp_end(byte[] new_ctg_ttl) {
		if (cur_cat_ttl != ByteAry_.Empty && cur_cat_id != -1)
	...
	int Cur_cat_id_find(byte[] ttl) {
		while (true) {
			int compare = ByteAry_.Compare(ttl, 0, ttl.length, name_id_rdr.Bfr(), name_id_rdr.Key_pos_bgn(), name_id_rdr.Key_pos_end());
			switch (compare) {
