...
public class ByteAryFmtr_tst {
	...
	@Test  public void Simple() {
		ByteAryFmtr fmtr = ByteAryFmtr.new_("0~{key1}1~{key2}2", "key1", "key2");
		Tfds.Eq("0.1,2", fmtr.Bld_str_many(".", ","));
	}
	@Test  public void Cmd() {
		ByteAryFmtr_tst_mok mok = new ByteAryFmtr_tst_mok();
		ByteAryFmtr fmtr = ByteAryFmtr.new_("0~{key1}2~{<>3<>}4", "key1").Eval_mgr_(mok);
...
class ByteAryFmtr_fxt {
	public ByteAryFmtr_fxt Clear() {
		if (fmtr == null) {
			fmtr = ByteAryFmtr.new_();
		}
		return this;
	}	private ByteAryFmtr fmtr;
