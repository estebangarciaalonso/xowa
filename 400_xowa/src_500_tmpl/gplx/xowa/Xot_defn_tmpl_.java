...
public class Xot_defn_tmpl_ {
	public static Xot_invk CopyNew(Xop_ctx ctx, Xot_defn_tmpl orig_defn, Xot_invk orig, Xot_invk caller, byte[] src) {	// SEE:NOTE_1
		...
		Xot_invk_temp rv = new Xot_invk_temp(orig_src, orig.Name_tkn(), caller.Src_bgn(), caller.Src_end());
	...
	private static Arg_itm_tkn MakeItm(boolean val_tkn, Xop_ctx ctx, Xop_tkn_mkr tkn_mkr, byte[] src, Arg_itm_tkn orig, Xot_invk caller, Arg_nde_tkn orig_arg) {
		...
		byte[] rv_ary = orig_arg.KeyTkn_exists() && val_tkn ? arg_bfr.XtoAryAndClearAndTrim() : arg_bfr.XtoAryAndClear();	// NOTE: need to trim val if arg is keyd; EX.WP:Coord in Chernobyl disaster, Sahara
		rv.Dat_ary_(rv_ary);
		return rv;
	}
}
