...
public class Scrib_invoke_func_tst {
	...
	@Test  public void Preprocess() {
		...
		this.Exec_preprocess(Scrib_core.Frame_key_current	, "1", "c");
		this.Exec_preprocess(Scrib_core.Frame_key_current	, "2", "d");
		this.Exec_preprocess(Scrib_core.Frame_key_parent	, "1", "a");
		this.Exec_preprocess(Scrib_core.Frame_key_parent	, "2", "b");
	}
	@Test  public void ExpandTemplate() {
		this.Init_expandTemplate();
		fxt.Parser_fxt().Init_page_create("Template:Format", "{{{1}}}");
		fxt.Init_lua_module();
		fxt.Init_lua_rcvd_expandTemplate(Scrib_core.Frame_key_current	, "Format", KeyVal_.int_(1, "a"));
		fxt.Test_invoke("a");
	}
	@Test  public void ExpandTemplate_ns_name() {
		...
		fxt.Parser_fxt().Init_page_create("Template:Format", "{{{1}}}");
		fxt.Init_lua_module();
		fxt.Init_lua_rcvd_expandTemplate(Scrib_core.Frame_key_current	, "Template:Format", KeyVal_.int_(1, "a"));
		fxt.Test_invoke("a");
	}
