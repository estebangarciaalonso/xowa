...
public class Xpath_ {
	...
	static KeyValHash ExtractNodeVals(String xml, Err xmlErr) {
		...
		int pos = 0;
		while (true) {
			int eqPos = String_.FindFwd(xml, "=", pos);		if (eqPos == String_.NotFound) break;
			int q0Pos = String_.FindFwd(xml, "\"", eqPos + 1); if (q0Pos == String_.NotFound) throw xmlErr.Add("eqPos", eqPos);
			int q1Pos = String_.FindFwd(xml, "\"", q0Pos + 1); if (q1Pos == String_.NotFound) throw xmlErr.Add("q1Pos", q1Pos);
			int spPos = eqPos - 1;
			while (spPos > -1) {
