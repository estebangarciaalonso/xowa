...
public class Xop_amp_trie {
	...
	public static ByteTrieMgr_slim new_() {// REF.MW: Sanitizer|$wgHtmlEntities; NOTE:added apos
	...
	private static void Reg_char(ByteTrieMgr_slim trie, int char_int, String xml_name_str) {
		byte[] xml_name_bry = ByteAry_.new_ascii_(xml_name_str);
		Xop_amp_trie_itm itm = new Xop_amp_trie_itm(Xop_amp_trie_itm.Tid_name, char_int, xml_name_bry);
		byte[] key = ByteAry_.Mid(xml_name_bry, 1, xml_name_bry.length); // ignore & for purpose of trie; EX: "amp;"; NOTE: must keep trailing ";" else "&amp " will be valid;
		trie.Add(key, itm);
	}
	private static void Reg_prefix(ByteTrieMgr_slim trie, byte prefix_type, String prefix) {
		byte[] prefix_ary = ByteAry_.new_ascii_(prefix);
