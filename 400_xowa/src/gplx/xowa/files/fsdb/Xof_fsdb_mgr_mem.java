...
import gplx.fsdb.*; import gplx.xowa.files.wiki_orig.*; import gplx.xowa.files.fsdb.qrys.*; import gplx.xowa.files.bins.*;
...
public class Xof_fsdb_mgr_mem implements Xof_fsdb_mgr, Xof_bin_wkr {
	...
	public void Thm_insert(Fsd_thm_itm rv, byte[] dir, byte[] fil, int ext_id, int thm_w, int thm_h, double thumbtime, int page, DateAdp modified, String hash, long bin_len, gplx.ios.Io_stream_rdr bin_rdr) {
		byte[] key = Key_bld_thm(dir, fil, thm_w, thumbtime);
		byte[] bin = gplx.ios.Io_stream_rdr_.Load_all_as_bry(bin_rdr);
		Fsdb_thm_itm_mem itm = new Fsdb_thm_itm_mem();
	...
	public void Img_insert(Fsd_img_itm rv, byte[] dir, byte[] fil, int ext_id, int img_w, int img_h, DateAdp modified, String hash, long bin_len, gplx.ios.Io_stream_rdr bin_rdr) {
		byte[] key = Key_bld_fil(dir, fil);
		byte[] bin = gplx.ios.Io_stream_rdr_.Load_all_as_bry(bin_rdr);
		Fsdb_img_itm_mem itm = new Fsdb_img_itm_mem();
...
class Fsdb_thm_itm_mem extends Fsd_thm_itm implements Fsdb_mem_itm {	public byte[] Mem_key() {return mem_key;} private byte[] mem_key;
...
class Fsdb_img_itm_mem extends Fsd_img_itm implements Fsdb_mem_itm {		public byte[] Mem_key() {return mem_key;} private byte[] mem_key;
	public byte[] Wiki() {return wiki;} private byte[] wiki;
	public byte[] Ttl() {return ttl;} private byte[] ttl;
	public byte[] Bin() {return bin;} private byte[] bin;
...
class Fsdb_fil_itm_mem extends Fsd_fil_itm implements Fsdb_mem_itm {		public byte[] Mem_key() {return mem_key;} private byte[] mem_key;
	public byte[] Wiki() {return wiki;} private byte[] wiki;
	public byte[] Ttl() {return ttl;} private byte[] ttl;
	public byte[] Bin() {return bin;} private byte[] bin;
