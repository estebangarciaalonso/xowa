...
public class Fsm_atr_fil implements RlsAble {
	...
	public Fsm_atr_fil(Fsm_abc_mgr abc_mgr, Io_url io_url, boolean create) {
		this.abc_mgr = abc_mgr;
		conn = create ? Db_conn_bldr.I.New(Db_conn_bldr_type, io_url) : Db_conn_bldr.I.Get(Db_conn_bldr_type, io_url);
		int db_id = 0; boolean version_is_1 = Bool_.Y; 
		tbl_dir.Conn_(conn, db_id, version_is_1);
		tbl_fil.Conn_(conn, db_id, version_is_1);
		tbl_thm.Conn_(conn, db_id, version_is_1, this);
		if (create) {
			tbl_dir.Create_table();
			tbl_fil.Create_table();
			tbl_thm.Create_table();
		}
	}
	...
	public static Fsm_atr_fil make_(Fsm_abc_mgr abc_mgr, int id, Io_url url, String path_bgn) {
		Fsm_atr_fil rv = new Fsm_atr_fil(abc_mgr, url, true);
		rv.id = id;
		rv.url = url;
