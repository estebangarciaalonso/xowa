...
public class Pp_pages_nde_tst {
	...
	@Test  public void Recursive_index() {	// PURPOSE: handle recursive calls on index; EX: en.w:Poems_of_Italy:_selections_from_the_Odes_of_Giosue_Carducci/Before_the_Old_Castle_of_Verona; DATE:2014-01-19
		fxt.ini_page_create("Index:A", "<pages index=A/>");
		fxt.tst_Parse_page_wiki_str("<pages index=\"A\" from=1 to=1 />", "");
	}
...
//		}
	@Test  public void Section_failed_when_xnde() {	// PURPOSE: section failed to be retrieved if preceding xnde; DATE:2014-01-15
		...
		String main_txt = "<pages index=\"A\" from=1 to=1 fromsection='sect_0' tosection='sect_0' />"; 
		fxt.tst_Parse_page_wiki_str(main_txt, String_.Concat_lines_nl
		(	"<p>b"
		,	"</p>"
		));
	...
	@Test  public void Various() {
		...
		fxt.tst_Parse_page_wiki_str("<pages index=\"A\" include='1-2,4' />", "<p>abd\n</p>\n");				// include
		fxt.tst_Parse_page_wiki_str("<pages index=\"A\" from=1 to=5 exclude='3' />", "<p>abde\n</p>\n");	// exclude
		fxt.tst_Parse_page_wiki_str("<pages index=\"A\" include=5 from=2 to=4 />", "<p>bcde\n</p>\n");		// include should be sorted
		fxt.tst_Parse_page_wiki_str("<pages index=\"A\" from=1 to=5 step=2 />", "<p>ace\n</p>\n");			// step
		fxt.tst_Parse_page_wiki_str("<pages index=\"A\" from=1 to=5 step=4 />", "<p>ae\n</p>\n");			// step
		...
		fxt.tst_Parse_page_wiki_str("<pages index=\"A\" to=3/>", "<p>abc\n</p>\n");							// from omitted
		fxt.tst_Parse_page_wiki_str("<pages index=\"A\" from=3/>", "<p>cde\n</p>\n");						// to omitted
		fxt.tst_Parse_page_wiki_str("<pages index=\"A\" from='' to=3 />", "<p>abc\n</p>\n");				// from is blank
		fxt.tst_Parse_page_wiki_str("<pages index=\"A\" from=3 to=''/>", "<p>cde\n</p>\n");					// to is blank
		fxt.tst_Parse_page_wiki_str("<pages index=\"A\" from=3 to='4.' />", "<p>cd\n</p>\n");				// allow decimal-like number; EX:en.w:Haworth's/Chapter_XIX; DATE:2014-01-19
		fxt.tst_Parse_page_wiki_str("<pages index=\"A\" from=1 to=5 exclude=''3' />", "<p>abcde\n</p>\n");	// exclude is invalid; EX:fr.s:Sanguis_martyrum/Premi�re_partie/I DATE:2014-01-18
	}
