...
import gplx.dbs.*; import gplx.cache.*; import gplx.fsdb.tbls.*;
public class Fsdb_vlm_data implements CompareAble {
	public int Id() {return id;} private int id;
	public String Key() {return key;} private String key;
	public Io_url Url() {return url;} private Io_url url;
	public byte[] Rng_bgn() {return rng_bgn;} public Fsdb_vlm_data Rng_bgn_(byte[] v) {rng_bgn = v; return this;} private byte[] rng_bgn;
	public Db_provider Provider() {return provider;} private Db_provider provider;
	public Fsdb_vlm_data Init(Io_url url, int id, String key, byte[] rng_bgn) {
	...
	public void Init_by_provider(Db_provider p) {
		...
		Fsdb_wmf_img_tbl.Create_table(p);
		Fsdb_bin_tbl.Create_table(p);
		Fsdb_cfg_tbl.Create_table(p);
	}
	private Gfo_cache_mgr dir_cache = new Gfo_cache_mgr();
	public void Img_insert(Fsdb_wmf_img_itm rv, String dir, String fil, int ext_id, int width, int thumbtime, boolean img_is_orig, int height, DateAdp modified, String sha1, byte[] bin) {
		int dir_id = -1;
		...
		if (fil_id == 0) {
			...
			Fsdb_fil_tbl.Insert(provider, fil_id, dir_id, fil, ext_id);
		}
	...
	public int Img_insert(int dir_id, int fil_id, int ext_id, int width, int thumbtime, boolean img_is_orig, int height, DateAdp modified, String sha1, byte[] bin) {return Img_insert(Fsdb_wmf_img_tbl.Insert_stmt(provider), Fsdb_bin_tbl.Insert_stmt(provider), dir_id, fil_id, ext_id, width, thumbtime, img_is_orig, height, modified, sha1, bin);}
	public int Img_insert(Db_stmt img_stmt, Db_stmt bin_stmt, int dir_id, int fil_id, int ext_id, int width, int thumbtime, boolean img_is_orig, int height, DateAdp modified, String sha1, byte[] bin) {
		...
		Fsdb_wmf_img_tbl.Insert(img_stmt, img_id, fil_id, width, thumbtime, img_is_orig, height, bin.length, modified_str, sha1);
		Fsdb_bin_tbl.Insert(bin_stmt, img_id, bin);
		return img_id;
	}
	public void Fil_delete(Db_provider p, int fil_id) {
		Fsdb_wmf_img_itm[] itms = Fsdb_wmf_img_tbl.Select_itms_by_owner(p, fil_id);
		int len = itms.length;
		Db_stmt stmt = Fsdb_wmf_img_tbl.Delete_by_id_stmt(p);
		for (int i = 0; i < len; i++)
			Fsdb_wmf_img_tbl.Delete_by_id(stmt, itms[i].Id());
		Fsdb_fil_tbl.Delete_by_id(p, fil_id);
	}
	public void Img_delete(Db_provider p, int img_id) {
		Fsdb_wmf_img_tbl.Delete_by_id(p, img_id);
		Fsdb_bin_tbl.Delete(p, img_id);
	}
	public void Update_fil_bin(int fil_id, boolean img_is_orig, int sub_id, byte[] data) {
		Fsdb_wmf_img_tbl.Update(provider, sub_id, fil_id, 0, 0, img_is_orig, 0, data.length, "", "");
		Fsdb_bin_tbl.Update(provider, sub_id, data);
	}
	public int compareTo(Object obj) {
		Fsdb_vlm_data comp = (Fsdb_vlm_data)obj;
		return ByteAry_.Compare(rng_bgn, comp.rng_bgn);
	}
	public static Fsdb_vlm_data make_root_by_dir(Io_url dir) {return make_root_(url_root_(dir));}
	public static Fsdb_vlm_data make_root_(Io_url url) {
		Fsdb_vlm_data rv = new Fsdb_vlm_data().Init(url, 0, "", Path_bgn_0);
