...
public class Xosrv_msg_rdr {
	...
	public Xosrv_msg Read() {
		...
		int body_len = ByteAry_.X_to_int_or(header_bry,  2, 12, -1); 	if (body_len == -1)					return Xosrv_msg.fail_("body_len is not number; body_len:{0}", String_.new_utf8_(header_bry,  2, 23));
		int cksum    = ByteAry_.X_to_int_or(header_bry, 13, 23, -1);	if (cksum == -1)					return Xosrv_msg.fail_("checksum is not number; cksum:{0}", String_.new_utf8_(header_bry, 13, 23));
		...
		byte[] msg_text		= ByteAry_.Mid(body_bry, fld_bgn.Val(), body_len);
	...
	private static byte[] Read_fld(byte[] bry, int bry_len, IntRef fld_bgn, BoolRef fail_ref, StringRef fld_ref) {
		int fld_end = Byte_ary_finder.Find_fwd(bry, Byte_ascii.Pipe, fld_bgn.Val(), bry_len);
	...
	private static Xosrv_msg Read_fld_fail(StringRef fld_name, byte[] body_bry) {return Xosrv_msg.fail_("pipe not found for " + fld_name.Val() + "; body:{0}", String_.new_utf8_(body_bry));}
