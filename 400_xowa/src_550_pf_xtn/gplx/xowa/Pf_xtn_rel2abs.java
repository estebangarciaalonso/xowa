...
class Pf_xtn_rel2abs extends Pf_func_base {
	...
	private static void qry_bgns_with_init() {
		...
		qry_bgns_with.Add(Byte_ascii.Dot, IntRef.new_(Id_dot));
		qry_bgns_with.Add(Ary_dot_slash, IntRef.new_(Id_dot_slash));
		qry_bgns_with.Add(Ary_dot_dot, IntRef.new_(Id_dot_dot));
		qry_bgns_with.Add(Ary_dot_dot_slash, IntRef.new_(Id_dot_dot_slash));
					...
					switch (seg_len) {
						...
						case 2:	
							if (dot_mode) {				// "/../"; pop seg_ary
								seg_pos -= 2;
								if (seg_pos < 0) return ByteAry_.Empty; // FUTURE: return MediaWiki error
							}
