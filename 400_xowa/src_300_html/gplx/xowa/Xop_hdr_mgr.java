...
public class Xop_hdr_mgr {
	private Xow_wiki wiki; private Xoa_page page;
	private Xop_hdr_tkn[] hdrs_ary = new Xop_hdr_tkn[0]; private int hdrs_max, hdrs_len;
	private HashAdp hdrs_hash = HashAdp_.new_(); private ByteAryBfr hdrs_bfr = ByteAryBfr.reset_(255); private ByteAryRef hdrs_ref = ByteAryRef.null_();
	public Xop_hdr_mgr(Xow_wiki wiki, Xoa_page page) {this.wiki = wiki; this.page = page;}
	...
	private void Reg(Xop_ctx ctx, Xop_hdr_tkn hdr, byte[] src) {
		...
		encoder.Encode(enc_bfr, raw_bfr.Bry(), 0, raw_bfr.Len());
		...
		hdr.Hdr_toc_text_(gplx.xowa.html.tocs.Xop_toc_mgr.Toc_text(ctx, page, src, hdr));
		raw_bfr.Mkr_rls(); enc_bfr.Mkr_rls();
	}
	private void Id_bld_recurse(ByteAryBfr raw_bfr, byte[] src, Xop_tkn_itm tkn) {
