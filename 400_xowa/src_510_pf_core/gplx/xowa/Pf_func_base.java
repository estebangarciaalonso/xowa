	...
	public byte[] Eval_argx_or_null(Xop_ctx ctx, byte[] src, Xot_invk caller, Xot_invk self, byte[] func_name) {
		if (argx_dat == ByteAry_.Empty) {
			...
			if (subs_len == 0) {	// no subs; either {{#func}} or {{#func:}}
				...
				int colon_pos = Byte_ary_finder.Find_bwd(src, Byte_ascii.Colon, self.Src_end(), src_bgn);	// look for ":"; NOTE: used to be src_bgn - 1, but this would always search one character too many; DATE:2014-02-11
