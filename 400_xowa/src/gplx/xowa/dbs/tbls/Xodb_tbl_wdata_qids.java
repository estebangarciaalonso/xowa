...
import gplx.dbs.*;
public class Xodb_tbl_wdata_qids {
	...
	public byte[] Select_qid(Db_provider p, byte[] src_wiki, byte[] src_ns, byte[] src_ttl) {
		...
		try {
			...
			String rv = (String)stmt.Val_str_by_bry_(src_wiki).Val_int_(ByteAry_.XtoInt(src_ns)).Val_str_by_bry_(src_ttl).Exec_select_val();
			return rv == null ? null : ByteAry_.new_utf8_(rv);
		} finally {stmt.Rls();}
	}
