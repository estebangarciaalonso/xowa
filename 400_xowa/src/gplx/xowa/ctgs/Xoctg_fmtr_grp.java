...
class Xoctg_fmtr_grp implements ByteAryFmtrArg {
	...
	public void XferAry(ByteAryBfr bfr, int xfer_idx) {
		if (mgr.Grps_enabled()) {
			...
			byte[] ttl_char_0 = ByteAry_.Empty; int col_idx = -1; boolean col_bgn_needed = true;
			while (cur_idx < len) {
			...
			itms_fmtr.Init_from_grp(ByteAry_.Empty, itms_list.Bgn());
			html_none.Bld_bfr_many(bfr, itms_fmtr);
		}
	}
	ByteAryFmtr html_none = ByteAryFmtr.new_(String_.Concat_lines_nl_skipLast
		(	""
		,	"~{itms}"
		), 	"itms"
		);
	ByteAryFmtr html_tbl = ByteAryFmtr.new_(String_.Concat_lines_nl_skipLast
		(	""
		,	"          <h3>~{grp_name}</h3>"
		,	"          <ul>~{itms}"
		,	"          </ul>"
	...
	ByteAryFmtr html_col_bgn = ByteAryFmtr.new_(String_.Concat_lines_nl_skipLast
		(	""
		,	"        <td style=\"width: ~{width}%;\">"
		), 	"width"
		);
	ByteAryFmtr html_col_end = ByteAryFmtr.new_(String_.Concat_lines_nl_skipLast
		(	""
		,	"        </td>"
		));
