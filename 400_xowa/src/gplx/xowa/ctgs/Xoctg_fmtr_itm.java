...
abstract class Xoctg_fmtr_itm_base implements Xoctg_fmtr_itm {
	...
	}	protected Xow_wiki wiki; Xol_lang lang; Xoctg_view_ctg ctg; protected int len; protected Xoh_href_parser href_parser; protected ByteAryFmtr html_itm, html_itm_missing; protected Xoctg_view_grp list; protected Xow_msg_mgr msg_mgr;
	...
	@gplx.Virtual public void XferAry(ByteAryBfr bfr, int idx) {
		for (int i = col_bgn; i < len; i++) {
			...
			if (!ByteAry_.HasAtBgn(itm_sortkey, ttl_char_0, 0, itm_sortkey.length)) {
				grp_end_idx = i;
	...
	@gplx.Virtual public void Bld_html(ByteAryBfr bfr, Xow_wiki wiki, Xoctg_view_itm itm, Xoa_ttl ttl, byte[] ttl_page, Xoh_href_parser href_parser, ByteAryFmtr html_itm) {
		...
		byte[] itm_atr_cls = Xoh_lnki_wtr.Lnki_cls_visited(history_mgr, wiki.Key_bry(), ttl.Page_db());
...
class Xoctg_fmtr_itm_subc extends Xoctg_fmtr_itm_base {
	@Override public void Bld_html(ByteAryBfr bfr, Xow_wiki wiki, Xoctg_view_itm itm, Xoa_ttl ttl, byte[] ttl_page, Xoh_href_parser href_parser, ByteAryFmtr html_itm) {
		byte[] itm_href = href_parser.Build_to_bry(ttl, wiki);
	...
	byte[] Bld_contains_text(int sub_ctgs, int sub_pages, int sub_files) {
		if (sub_ctgs == 0 && sub_pages == 0 && sub_files == 0) return ByteAry_.Empty;
