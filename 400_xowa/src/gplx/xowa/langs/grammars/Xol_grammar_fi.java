...
public class Xol_grammar_fi implements Xol_grammar {
	...
	public boolean Grammar_eval(ByteAryBfr bfr, Xol_lang lang, byte[] word, byte[] type) {
		...
		if (aou && ByteAry_.HasAtEnd(lower, Xoa_url_parser.Bry_wiki_name))
			aou = false;			
	...
	private static final byte[] Bry_sta_y = ByteAry_.new_ascii_("sta"), Bry_sta_n = ByteAry_.new_utf8_("stä"), Bry_a_y = ByteAry_.new_ascii_("a"), Bry_a_n = ByteAry_.new_utf8_("ä"), Bry_ssa_y = ByteAry_.new_ascii_("ssa"), Bry_ssa_n = ByteAry_.new_utf8_("ssä");
	static final byte Trie_vh_back = 0, Trie_vh_front = 1;
	private static ByteTrieMgr_slim trie_vh = ByteTrieMgr_slim.cs_().Add_byteVal_strAry(Trie_vh_back, "a", "o", "u").Add_byteVal_strAry(Trie_vh_front, "ä", "ö", "y");
}
