...
public class References_nde implements Xox_xnde, Xop_xnde_atr_parser {
	public byte[] Group() {return group;} public References_nde Group_(byte[] v) {group = v; return this;} private byte[] group = ByteAry_.Empty;
			...
			for (int i = 0; i < xnde_subs; i++) {
				...
				if (tkn.Tkn_tid() == Xop_tkn_itm_.Tid_xnde) {
					...
					if (ref_xnde.Tag().Id() == Xop_xnde_tag_.Tid_ref) {
						Ref_nde ref_itm = new Ref_nde().Head_(true).Group_(group);
