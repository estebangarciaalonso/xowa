...
public class Fsdb_xtn_thm_tbl {
	...
	public static void Insert(Db_provider p, int id, int thm_owner, int width, int thumbtime, int height, long size, DateAdp modified, String hash) {
		Db_stmt stmt = Insert_stmt(p);
		try {Insert(stmt, id, thm_owner, width, thumbtime, height, size, modified, hash);}
		finally {stmt.Rls();}
	}
	public static Db_stmt Insert_stmt(Db_provider p) {return Db_stmt_.new_insert_(p, Tbl_name, Fld_thm_id, Fld_thm_owner, Fld_thm_width, Fld_thm_thumbtime, Fld_thm_height, Fld_thm_size, Fld_thm_modified, Fld_thm_hash);}
	public static void Insert(Db_stmt stmt, int id, int thm_owner, int width, int thumbtime, int height, long size, DateAdp modified, String hash) {
		stmt.Clear()
		.Val_int_(id)
		.Val_int_(thm_owner)
		.Val_int_(width)
	...
	public static Fsdb_xtn_thm_itm Select_itm_by_id(Db_provider p, int thm_id) {
		Db_qry qry = Db_qry_.select_().From_(Tbl_name).Cols_(Fld_thm_id, Fld_thm_owner, Fld_thm_width, Fld_thm_thumbtime, Fld_thm_height, Fld_thm_size, Fld_thm_modified, Fld_thm_hash)
			.Where_(Db_crt_.eq_(Fld_thm_id, thm_id));
		DataRdr rdr = DataRdr_.Null;
		try {
			rdr = p.Exec_qry_as_rdr(qry);
			if (rdr.MoveNextPeer()) {
				Fsdb_xtn_thm_itm itm = new Fsdb_xtn_thm_itm().Init_by_db
				( rdr.ReadInt(Fld_thm_id), rdr.ReadInt(Fld_thm_owner), rdr.ReadInt(Fld_thm_width), rdr.ReadInt(Fld_thm_thumbtime) / Thumbtime_multiplier
	...
	public static Fsdb_xtn_thm_itm Select_itm_by_fil_width(Db_provider p, int owner, int width, int thumbtime) {
		Db_qry qry = Db_qry_.select_().From_(Tbl_name).Cols_(Fld_thm_id, Fld_thm_size, Fld_thm_modified, Fld_thm_hash, Fld_thm_width, Fld_thm_height, Fld_thm_thumbtime)
		...
		try {
			rdr = p.Exec_qry_as_rdr(qry);
			if (rdr.MoveNextPeer())
				return new Fsdb_xtn_thm_itm().Init_by_db
				( rdr.ReadInt(Fld_thm_id), owner, rdr.ReadInt(Fld_thm_width), rdr.ReadInt(Fld_thm_thumbtime) / Thumbtime_multiplier
				, rdr.ReadInt(Fld_thm_height), rdr.ReadLong(Fld_thm_size), rdr.ReadStr(Fld_thm_modified), rdr.ReadStr(Fld_thm_hash)
				);
			else
				return Fsdb_xtn_thm_itm.Null;
	...
	, Fld_thm_height = "thm_height", Fld_thm_size = "thm_size", Fld_thm_modified = "thm_modified", Fld_thm_hash = "thm_hash";
	...
	;
	private static final int Thumbtime_multiplier = 1000;	// store thumbtime in ms; EX: 2 secs -> 2000
