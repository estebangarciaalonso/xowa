...
import org.junit.*;
public class Xtn_ref_mgr_tst {
	Xtn_ref_mgr_fxt fxt = new Xtn_ref_mgr_fxt();
...
class Xtn_ref_mgr_fxt {
	Xtn_ref_mgr ref_mgr = new Xtn_ref_mgr();
	...
	public Xtn_ref_mgr_fxt run_Add_follow(String follow)		{return run_Add_grp(ByteAry_.Empty, ByteAry_.new_utf8_(follow), ByteAry_.Ary(ByteAry_.Empty));}
	public Xtn_ref_mgr_fxt run_Add(String... name_ary)	{return run_Add_grp(ByteAry_.Empty, ByteAry_.Empty, ByteAry_.Ary(name_ary));}
	public Xtn_ref_mgr_fxt run_Add_grp(String grp, String... name_ary) {return run_Add_grp(ByteAry_.new_utf8_(grp), ByteAry_.Empty, ByteAry_.Ary(name_ary));}
	public void Clear() {ref_mgr.Grps_clear();}
	Xtn_ref_mgr_fxt run_Add_grp(byte[] grp_name, byte[] follow, byte[]... name_ary) {
		for (int i = 0; i < name_ary.length; i++) {
			byte[] name = name_ary[i];
			Xtn_ref_nde itm = new Xtn_ref_nde().Name_(name);
			ref_mgr.Grps_add(grp_name, name, follow, itm);
		}
		return this;
	}
	public Xtn_ref_mgr_fxt tst(Xtn_ref_nde_chkr... chkr_ary) {return tst_grp(ref_mgr.Lst_get(ByteAry_.Empty, 0), chkr_ary);}
	public Xtn_ref_mgr_fxt tst_grp(String grp_name, Xtn_ref_nde_chkr... chkr_ary) {return tst_grp(ref_mgr.Lst_get(ByteAry_.new_ascii_(grp_name), 0), chkr_ary);}
	public Xtn_ref_mgr_fxt tst_grp(Xtn_ref_lst lst, Xtn_ref_nde_chkr... chkr_ary) {
		int itms_len = lst.Itms_len();
		Xtn_ref_nde[] actl = new Xtn_ref_nde[itms_len];
...
class Xtn_ref_nde_chkr implements Tst_chkr {
	public Class<?> TypeOf() {return Xtn_ref_nde.class;}
	...
	public Xtn_ref_nde_chkr Idx_minor_follow_() {idx_minor = Xtn_ref_nde.Idx_minor_follow; return this;}
	...
	public int Chk(Tst_mgr mgr, String path, Object actl_obj) {
		Xtn_ref_nde actl = (Xtn_ref_nde)actl_obj;
		int rv = 0;
	...
	Xtn_ref_nde[] XtoAry(Xtn_ref_nde itm) {
		int len = itm.Related_len();
		Xtn_ref_nde[] rv = new Xtn_ref_nde[len];
		for (int i = 0; i < len; i++)
