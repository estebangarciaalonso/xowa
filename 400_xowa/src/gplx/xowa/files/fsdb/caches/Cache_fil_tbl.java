...
class Cache_fil_tbl {		
	...
	public String Db_save(Cache_fil_itm itm) {
		try {
			...
			switch (itm.Cmd_mode()) {
				case Db_cmd_mode.Tid_create:	stmt.Clear().Val_int(itm.Uid()); Db_save_modify(stmt, itm); stmt.Exec_insert(); break;
				case Db_cmd_mode.Tid_update:	stmt.Clear();					  Db_save_modify(stmt, itm); stmt.Val_int(itm.Uid()).Exec_update(); break;
