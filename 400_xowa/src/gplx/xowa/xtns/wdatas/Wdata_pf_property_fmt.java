...
public class Wdata_pf_property_fmt {
	...
	public void Init() {
		...
		separator = comma;
		value_template = ByteAry_.new_ascii_("{{{value}}} {{{qualifiers}}}{{{references}}}");
		qualifier_template = ByteAry_.new_ascii_("{{{1}}} {{{2}}}");
		qualifier_separator = comma;
		qualifiers_wrapper = ByteAry_.new_ascii_("{{{1}}}");
		qualifiers_template = ByteAry_.Empty;
		qualifiers_value_separator = comma;
		reference_keyvalue_template = ByteAry_.new_ascii_("{{{1}}} {{{2}}}");
		reference_wrapper = ByteAry_.new_ascii_("<ref>{{{1}}}</ref>");
		references_wrapper = ByteAry_.new_ascii_("{{{1}}}");
		reference_keyvalue_separator = comma;
		reference_template = ByteAry_.Empty;
		reference_value_separator = comma;
		reference_separator = ByteAry_.Empty;	// "a separator to use between each reference"
	}
}
