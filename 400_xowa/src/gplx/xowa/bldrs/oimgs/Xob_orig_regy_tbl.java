...
class Xob_orig_regy_tbl {
	...
	private static void Create_data__update_page(Db_provider cur, byte wiki_tid, Io_url join) {
	...
	private static void Create_data__update_redirect(Db_provider cur, byte wiki_tid, Io_url join) {
		Sqlite_engine_.Db_attach(cur, "join_db", join.Raw());
		cur.Exec_sql(String_.Format(Sql_update_redirect_fmt, wiki_tid));
		Sqlite_engine_.Db_detach(cur, "join_db");
	}
	private static void Create_data__update_image(Db_provider cur, byte wiki_tid, Io_url join) {
		Sqlite_engine_.Db_attach(cur, "join_db", join.Raw());
