...
class Xop_tab_lxr implements Xop_lxr {
	...
	public void Ctor_lxr(Xow_wiki wiki, ByteTrieMgr_fast coreTrie) {
		coreTrie.Add(Byte_ascii.Tab, this);
		coreTrie.Add(Xop_tab_tkn.Bry_tab_ent, this);
	}	
	public int MakeTkn(Xop_ctx ctx, Xop_tkn_mkr tkn_mkr, Xop_root_tkn root, byte[] src, int srcLen, int bgn_pos, int cur_pos) {
		cur_pos = Xop_lxr_.Find_fwd_while(src, srcLen, cur_pos, Byte_ascii.Tab);
