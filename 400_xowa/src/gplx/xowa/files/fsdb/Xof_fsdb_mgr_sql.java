...
import gplx.dbs.*; import gplx.dbs.engines.sqlite.*; import gplx.fsdb.*; import gplx.xowa.files.wiki_orig.*; import gplx.xowa.files.bins.*; import gplx.xowa.files.fsdb.qrys.*; import gplx.xowa.files.fsdb.caches.*;
	...
	public int Patch_upright() {
		if (patch_upright_null) {
			if (mnt_mgr.Abc_mgr_len() > 0) {
				Fsm_cfg_grp cfg_grp = mnt_mgr.Abc_mgr_at(0).Cfg_mgr().Grps_get_or_load(Xof_fsdb_mgr_cfg.Grp_xowa);
...
//			Xof_qry_wkr_xowa qry_xowa = new Xof_qry_wkr_xowa(new Xof_wiki_finder(wiki.App().Wiki_mgr().Get_by_key_or_make(Xow_wiki_.Domain_commons_bry), wiki), new gplx.xowa.files.fsdb.qrys.Xof_img_meta_wkr_xowa());
			Xof_qry_wkr_wmf_api qry_wmf_api = new Xof_qry_wkr_wmf_api(wiki, new Xoapi_orig_wmf());
...
//			bin_mgr.Find_to_url_as_bool
	...
	public Fsd_fil_itm Fil_select_bin(byte[] dir, byte[] fil, boolean is_thumb, int width, double thumbtime)	{return mnt_mgr.Fil_select_bin(dir, fil, is_thumb, width, thumbtime);}
	public boolean Thm_select_bin(byte[] dir, byte[] fil, Fsd_thm_itm thm)								{return mnt_mgr.Thm_select_bin(dir, fil, thm);}
