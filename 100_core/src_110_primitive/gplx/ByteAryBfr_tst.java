...
package gplx;
import org.junit.*;
public class ByteAryBfr_tst {
	ByteAryBfr bb = new ByteAryBfr(16);
	@Before public void setup() {bb.Clear();}
	@Test  public void AddByte() {
		bb = new ByteAryBfr(2);	// NOTE: make sure auto-expands
		tst_AddByte("a", "a", 2);
		tst_AddByte("b", "ab", 2);
		tst_AddByte("c", "abc", 4);
	}
...
//			ByteAryFmtr_IntBldr ib = new ByteAryFmtr_IntBldr();
...
//			bb.Add_int(l);
	...
	void tst_Add_bfr_trimEnd_and_clear(String raw, String expd) {
		ByteAryBfr tmp = ByteAryBfr.new_().Add_str(raw);
