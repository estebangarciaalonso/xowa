...
public class Xot_prm_tkn extends Xop_tkn_itm_base {
		...
		if (find_tkn != null) {	// NOTE: find_tkn defaults to null
			...
			if (find_tkn_static) {	// subs_are_static, so extract idx/key; EX: {{{a b}}} will have 3 subs which are all static; {{{a{{{1}}}b}}} will be dynamic
				...
				if (find_tkn_end - find_tkn_bgn > 0) {	// NOTE: handles empty find_tkns; EX: {{{|safesubst:}}}
					prm_idx = ByteAry_.XtoIntByPos(src, find_tkn_bgn, find_tkn_end, -1);			// parse as number first; note that bgn,end should not include ws; EX: " 1 " will fail
	...
	@Override public boolean Tmpl_evaluate(Xop_ctx ctx, byte[] src, Xot_invk caller, ByteAryBfr bfr) {
		if (!find_tkn_static) {
			...
			prm_idx = ByteAry_.XtoIntByPos(find_bfr.Bry(), 0, find_bfr.Bry_len(), -1);	// parse as number first
			if (prm_idx == -1)
