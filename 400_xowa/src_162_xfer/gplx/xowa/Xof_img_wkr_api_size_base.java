...
public abstract class Xof_img_wkr_api_size_base {
	public boolean Api_query_size(Xof_img_wkr_api_size_base_rslts rv, Xow_wiki wiki, byte[] ttl, int width, int height) {
		Xog_win_wtr gui_wtr = wiki.App().Gui_wtr();
		Xowf_repo_mgr repo_mgr = wiki.File_mgr().Repo_mgr();
		int len = repo_mgr.Repos_len();
		for (int i = 0; i < len; i++) {
			Xofw_repo_pair repo_pair = repo_mgr.Repos_get_at(i);
			if (Api_query_size_exec(rv, wiki, ttl, width, height, gui_wtr, repo_pair.Wiki_key())) return true;
		}
		gui_wtr.Warn_many(Xof_img_wkr_api_size_base_wmf.GRP_KEY, "download_failed", "download_failed: ~{0}", String_.new_utf8_(ttl));
		return false;
	}
	public abstract boolean Api_query_size_exec(Xof_img_wkr_api_size_base_rslts rv, Xow_wiki wiki, byte[] ttl, int width, int height, Xog_win_wtr gui_wtr, byte[] repo_wiki_key);
}
class Xof_img_wkr_api_size_base_wmf extends Xof_img_wkr_api_size_base {		
...
//				xrg.Prog_fmt_hdr_(); // NOTE: do not uncomment; api will reuse whatever's in place
	...
	public static boolean Parse_xml(Xof_img_wkr_api_size_base_rslts rv, Xog_win_wtr gui_wtr, byte[] xml) {
		...
		if (Parse_xml_val(parse_xml_rng, gui_wtr, xml, xml_len, pos, Bry_xml_width))
			rv.Orig_w_(ByteAry_.XtoIntByPos(xml, parse_xml_rng.Val_0(), parse_xml_rng.Val_1(), 0));

		if (Parse_xml_val(parse_xml_rng, gui_wtr, xml, xml_len, pos, Bry_xml_height))
			rv.Orig_h_(ByteAry_.XtoIntByPos(xml, parse_xml_rng.Val_0(), parse_xml_rng.Val_1(), 0));
