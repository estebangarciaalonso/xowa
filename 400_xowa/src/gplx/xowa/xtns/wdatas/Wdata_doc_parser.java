...
public class Wdata_doc_parser {
	...
	public OrderedHash Bld_props(byte[] src, Json_itm_ary claims_nde) {
		...
		for (int i = 0; i < len; i++) {
			...
			IntRef prop_itm_id = IntRef.new_(prop_itm.Pid());
	...
	private Wdata_prop_itm_core New_prop(byte[] src, Json_itm_nde prop_nde) {
		...
		for (int i = 0; i < len; i++) {
			...
			ByteVal bv = (ByteVal)Prop_key_hash.Get_by_mid(src, kv_key.Src_bgn(), kv_key.Src_end());
			if (bv == null) {Warn("invalid prop node: ~{0}", String_.new_utf8_safe_(src, kv_key.Src_bgn(), kv_key.Src_end())); return null;}
			switch (bv.Val()) {
				case Prop_tid_m:
