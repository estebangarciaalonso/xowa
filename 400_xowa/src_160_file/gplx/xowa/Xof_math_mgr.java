...
public class Xof_math_mgr implements GfoInvkAble {
	public Xof_math_mgr(Xoa_app app) {this.app = app;} private Xoa_app app;
	public ProcessAdp Cmd_convert_tex_to_dvi() {return cmd_convert_tex_to_dvi;} ProcessAdp cmd_convert_tex_to_dvi = new ProcessAdp();
	public ProcessAdp Cmd_convert_dvi_to_png() {return cmd_convert_dvi_to_png;} ProcessAdp cmd_convert_dvi_to_png = new ProcessAdp();
	...
	Io_url Make_math_dir(String wiki_key) {return app.Fsys_mgr().Root_dir().GenSubDir_nest("file", wiki_key, "math");}
	...
	Io_url Make_png_fil(Io_url math_dir, String hash) {
		String Math_dir_spr = math_dir.Info().DirSpr();
		tmpBfr.Clear().Add(math_dir.Raw())
			...
			.Add(hash).Add(".png")
			;
		return Io_url_.new_fil_(tmpBfr.XtoStrAndClear());
	}
	...
	static final String GRP_KEY = "xowa.math.mgr";
}
