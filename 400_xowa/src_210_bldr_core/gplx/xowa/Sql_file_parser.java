...
public class Sql_file_parser {
	...
	public Sql_file_parser Flds_req_idx_(int flds_all_len, int... idxs) {
		flds_all = new Sql_fld_itm[flds_all_len];
		int len = idxs.length;
		for (int i = 0; i < len; i++) {
			int idx = idxs[i];
			Sql_fld_itm itm = new Sql_fld_itm(idx, ByteAry_.Empty);
			flds_all[idx] = itm;
		} 
		return this;
	}
	...
	public void Parse(Gfo_usr_dlg usr_dlg) {
		...
		try {
			...
			ByteAryBfr fil_bfr = ByteAryBfr.new_(), val_bfr = ByteAryBfr.new_();
			...
			while (reading_file) {
				...
				switch (mode) {
					case Mode_sql_bgn:
						cur_pos = Byte_ary_finder.Find_fwd(bfr, Bry_insert_into, cur_pos);
						if (cur_pos == ByteAry_.NotFound || cur_pos > bfr_len) {reading_file = false; continue;}
						cur_pos = Byte_ary_finder.Find_fwd(bfr, Bry_values, cur_pos);
						if (cur_pos == ByteAry_.NotFound || cur_pos > bfr_len) throw Err_.new_("VALUES not found");	// something went wrong;
						mode = Mode_fld;
	...
	private static final byte[] Bry_insert_into = ByteAry_.new_utf8_("INSERT INTO "), Bry_values = ByteAry_.new_utf8_(" VALUES (");
