...
public class Articles_func extends Pf_func_base {
	...
	@Override public void Func_evaluate(Xop_ctx ctx, byte[] src, Xot_invk caller, Xot_invk self, ByteAryBfr bfr) {
		...
		Articles_html_xtn_itm xtn_itm = (Articles_html_xtn_itm)ctx.Page().Xtn_mgr().Get_or_null(Articles_html_xtn_itm.Const_key);
		if (xtn_itm == null) {
			xtn_itm = new Articles_html_xtn_itm();
			xtn_itm.Init(ctx.Wiki(), ctx.Page());
			ctx.Page().Xtn_mgr().Add(xtn_itm);
		}
		Parse(xtn_itm, argx);
	}
	private void Parse(Articles_html_xtn_itm xtn_itm, byte[] argx) {
		int pos = Byte_ary_finder.Find_fwd(argx, Const_dlm); if (pos == Byte_ary_finder.Not_found) return;
		byte[] ttl	= ByteAry_.Trim(ByteAry_.Mid(argx, 0, pos));
		byte[] text = ByteAry_.Trim(ByteAry_.Mid(argx, pos + Const_dlm.length));
...
class Articles_itm_fmtr implements ByteAryFmtrArg {
...
class Articles_html_xtn_itm implements Xoh_xtn_itm {
	...
	public byte[] Key() {return Const_key;} public static final byte[] Const_key = ByteAry_.new_utf8_("related_articles");
	...
	public void Exec() {
		...
		html_fmtr.Bld_bfr_many(page.Portal_div_xtn(), wiki.Msg_mgr().Val_by_key_obj("relatedarticles-title"), itms_fmtr);
	}
	private static final Articles_itm_fmtr itms_fmtr = Articles_itm_fmtr._;
	private static final ByteAryFmtr html_fmtr = ByteAryFmtr.new_(String_.Concat_lines_nl_skipLast
