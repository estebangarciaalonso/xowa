...
public class Xoh_ref_wtr {
	...
	ByteAryFmtrArg Itm_id(Xtn_ref_nde itm, boolean caller_is_ref) {
	...
	ByteAryFmtrArg Grp_id(Xtn_ref_nde itm) {
	...
	int List_len(Xtn_ref_nde itm) {
		int len = itm.Related_len();
		int rv = len;
		for (int i = 0; i < len; i++) {
			Xtn_ref_nde list_itm = itm.Related_get(i);
	...
	public void Xnde_references(Xoh_html_wtr wtr, Xop_ctx ctx, Xoh_opts opts, ByteAryBfr bfr, byte[] src, Xop_xnde_tkn xnde, int depth) {
		...
		for (int j = 0; j < itms_len; j++) {	// iterate over itms in grp
			Xtn_ref_nde head_itm = lst.Itms_get_at(j);
...
class Xoh_ref_list_fmtr implements ByteAryFmtrArg {
	public Xtn_ref_nde IdentifyTxt() {
		if (HasTxt(itm)) return itm;
		int itm_related_len = itm.Related_len();
		for (int i = 0; i < itm_related_len; i++) {
			Xtn_ref_nde rel = itm.Related_get(i);
			if (HasTxt(rel)) return rel;
		}
		return itm; // no itm has text; TODO:WARN
	}
	boolean HasTxt(Xtn_ref_nde v) {return v.Body() != null && v.Body().Root_src().length > 0;}
	public void Init(Xoo_ref opt, Xtn_ref_nde itm) {this.opt = opt; this.itm = itm;} Xtn_ref_nde itm; Xoo_ref opt;
	public void XferAry(ByteAryBfr trg, int idx) {
		...
		for (int i = 0; i < related_len; i++) {
			Xtn_ref_nde link_itm = itm.Related_get(i);
