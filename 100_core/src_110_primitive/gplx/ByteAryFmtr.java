...
*/
package gplx;
public class ByteAryFmtr {
	public byte[] Fmt() {return fmt;} private byte[] fmt = ByteAry_.Empty;
	public boolean Fmt_null() {return fmt.length == 0;}
	public ByteAryFmtr_eval_mgr Eval_mgr() {return eval_mgr;} public ByteAryFmtr Eval_mgr_(ByteAryFmtr_eval_mgr v) {eval_mgr = v; return this;} ByteAryFmtr_eval_mgr eval_mgr = ByteAryFmtr_eval_mgr_gfs._;
	public ByteAryFmtr Fmt_(byte[] v) {fmt = v; dirty = true; return this;} public ByteAryFmtr Fmt_(String v) {return Fmt_(ByteAry_.new_utf8_(v));}
	public ByteAryFmtr Keys_(String... ary) {
		if (keys == null)	keys = HashAdp_.new_();
		else				keys.Clear();
		int ary_len = ary.length;
		for (int i = 0; i < ary_len; i++)
			keys.Add(ByteAryRef.new_(ByteAry_.new_utf8_(ary[i])), IntVal.new_(i));
		dirty = true;
		return this;
	}	HashAdp keys = null;
	public void Bld_bfr(ByteAryBfr bfr, byte[]... args) {
		if (dirty) Compile(); 
	...
	public void Bld_bfr_none(ByteAryBfr bfr) {
		if (dirty) Compile(); 
		for (int i = 0; i < itms_len; i++) {
			ByteAryFmtrItm itm = itms[i];
			if (itm.Arg)
	...
	public void Bld_bfr(ByteAryBfr bfr, ByteAryFmtrArg... args) {
		if (dirty) Compile(); 
		for (int i = 0; i < itms_len; i++) {
			ByteAryFmtrItm itm = itms[i];
			if (itm.Arg)
	...
	public void Bld_bfr_one(ByteAryBfr bfr, Object val) {
		Bld_bfr_one_ary[0] = val;
		Bld_bfr_ary(bfr, Bld_bfr_one_ary);
	}	Object[] Bld_bfr_one_ary = new Object[1];
	public void Bld_bfr_many(ByteAryBfr bfr, Object... args) {Bld_bfr_ary(bfr, args);}
	public void Bld_bfr_ary(ByteAryBfr bfr, Object[] args) {
		if (dirty) Compile(); 
		int args_len = args.length;
		for (int i = 0; i < itms_len; i++) {
			ByteAryFmtrItm itm = itms[i];
	...
	public byte[] Bld_bry_none(ByteAryBfr bfr) {Bld_bfr_ary(bfr, Object_.Ary_empty); return bfr.XtoAryAndClear();}
	public byte[] Bld_bry_many(ByteAryBfr bfr, Object... args) {
		Bld_bfr_ary(bfr, args);
		return bfr.XtoAryAndClear();
	}
	public String Bld_str_many(ByteAryBfr bfr, String fmt, Object... args) {
	...
	public String Bld_str_many(String... args) {
		...
		return rv.XtoStr();
	}	private ByteAryFmtrItm[] itms; int itms_len;
	public byte[] Missing_bgn() {return missing_bgn;} public ByteAryFmtr Missing_bgn_(byte[] v) {missing_bgn = v; return this;} private byte[] missing_bgn = missing_bgn_static; static byte[] missing_bgn_static = ByteAry_.new_utf8_("~{"), missing_end_static = ByteAry_.new_utf8_("}");
	public byte[] Missing_end() {return missing_end;} public ByteAryFmtr Missing_end_(byte[] v) {missing_end = v; return this;} private byte[] missing_end = missing_end_static;
	public int Missing_adj() {return missing_adj;} public ByteAryFmtr Missing_adj_(int v) {missing_adj = v; return this;} int missing_adj;
	public boolean Fail_when_invalid_escapes() {return fail_when_invalid_escapes;} public ByteAryFmtr Fail_when_invalid_escapes_(boolean v) {fail_when_invalid_escapes = v; return this;} private boolean fail_when_invalid_escapes = true;
	public ByteAryFmtr Compile() {
		ByteAryBfr lkp_bfr = new ByteAryBfr(16);
		...
		while (true) {
			...
			if		(lkp_is_active) {
				if (cur_byte == char_arg_end) {
					if (lkp_is_numeric)
						list.Add(ByteAryFmtrItm.arg_(lkp_bfr.XtoInt(0) - baseInt));
					else {
						...
						if (idx_ref == null) {
							...
							list.Add(ByteAryFmtrItm.arg_(((IntVal)idx_ref).Val() - baseInt));
						}
					}
					lkp_is_active = false;
					lkp_bfr.Clear();
				...
				if (nxt_byte == char_arg_bgn) {
					if (trg_pos > 0) {list.Add(ByteAryFmtrItm.dat_(trg_bry, trg_pos)); trg_pos = 0;}	// something pending; add it to list
		...
		if (trg_pos > 0) {list.Add(ByteAryFmtrItm.dat_(trg_bry, trg_pos)); trg_pos = 0;}
		itms = (ByteAryFmtrItm[])list.XtoAry(ByteAryFmtrItm.class);
	...
	int Compile_eval_cmd(byte[] fmt, int fmt_len, int eval_lhs_bgn, ListAdp list) {
		...
		if (eval_rslt == null) eval_rslt = ByteAry_.Mid(fmt, eval_lhs_bgn - 2, eval_rhs_end);	// not found; return original argument
		list.Add(ByteAryFmtrItm.dat_bry_(eval_rslt));
		return eval_rhs_end;
	}
	static final String GRP_KEY = "gplx.ByteAryFmtr";
	...
	public static final ByteAryFmtr Null = new ByteAryFmtr().Fmt_("");
	public static ByteAryFmtr tmp_() {return new ByteAryFmtr().Fmt_("").Keys_();}
	public static ByteAryFmtr new_(String fmt, String... keys) {return new ByteAryFmtr().Fmt_(fmt).Keys_(keys);}	// NOTE: keys may seem redundant, but are needed to align ordinals with proc; EX: fmt may be "~{A} ~{B}" or "~{B} ~{A}"; call will always be Bld(a, b); passing in "A", "B" guarantees A is 0 and B is 1;
	public static ByteAryFmtr new_() {return new ByteAryFmtr();}
	public static ByteAryFmtr keys_(String... keys) {return new ByteAryFmtr().Keys_(keys);}
	public static ByteAryFmtr new_bry_(byte[] fmt, String... keys) {return new ByteAryFmtr().Fmt_(fmt).Keys_(keys);}
