...
public class Xoh_href_parser {
	...
	public void Parse(Xoh_href rv, byte[] raw, Xow_wiki wiki, byte[] cur_page) {
		...
		switch (tmp_url.Protocol_tid()) {
			case Gfo_url_parser.Protocol_http_tid:		// tmp_url is "http:"; use it and exit; do not do any substitutions EX: http://en.wikipedia.org
			case Gfo_url_parser.Protocol_https_tid:
				rv.Tid_(Xoh_href.Tid_http);
				return;
			case Gfo_url_parser.Protocol_file_tid:		// tmp_url is "file:"; remove it; NOTE: swt/mozilla automatically prepends "file://" to any protocol-less links; see NOTE_1 below
				int file_proto_len = tmp_url.Protocol_bry().length;
				bgn = file_slash_end = ByteAry_.While_fwd(raw, Byte_ascii.Slash, file_proto_len, raw_len);
	...
	public byte[] Build_to_bry(Xoa_ttl ttl, Xow_wiki wiki)				{Build_to_bfr(ttl, wiki, bfr_temp); return bfr_temp.XtoAryAndClear();}
	public void Build_to_bfr(byte[] raw, Xow_wiki wiki, ByteAryBfr bfr) {Build_to_bfr(Xoa_ttl.parse_(wiki, raw), wiki, bfr);}
	public void Build_to_bfr(Xoa_ttl ttl, Xow_wiki wiki, ByteAryBfr bfr) {
		byte[] page = ttl.Full_txt_raw();
		Xow_xwiki_itm xwiki = ttl.Wik_itm();
		if (xwiki == null) {								// not an xwiki; EX: [[wikt:Word]]
			if (ttl.Leaf_bgn() != ByteAry_.NotFound) {		// NOTE: this is strange logic to handle urls of for form [[../a]]; need to revisit why (a) only Raw has resolved title and (b) why it's not encoded
...
//				encoder.Encode(bfr_encoder, page, wik_txt.length + 1, page.length);
	...
	private void Build_to_bfr_page(Xoa_ttl ttl, byte[] ttl_full, int page_bgn) {
		int anch_bgn = Byte_ary_finder.Find_fwd(ttl_full, Byte_ascii.Hash);	// NOTE: cannot use Anch_bgn b/c Anch_bgn has bug with whitespace
		if (anch_bgn == ByteAry_.NotFound)	// no anchor; just add page
			...
			int page_end = Byte_ary_finder.Find_bwd_last_ws(ttl_full, anch_bgn);		// first 1st ws before #; handles multiple ws
	...
	public static final byte[] Href_http_bry = ByteAry_.new_utf8_(Href_http_str), Href_file_bry = ByteAry_.new_ascii_(Href_file_str), Href_site_bry = ByteAry_.new_ascii_(Href_site_str), Href_wiki_bry = ByteAry_.new_ascii_(Href_wiki_str);
	...
	private static final byte[] Seg_null_bry = ByteAry_.new_ascii_("/null/"), Seg_wiki_bry = ByteAry_.new_ascii_(Href_wiki_str), Seg_site_bry = ByteAry_.new_ascii_(Href_site_str), Seg_xcmd_bry = ByteAry_.new_ascii_(Href_xcmd_str);
	...
	private static void Parse_wiki(Xoh_href rv, Url_encoder encoder, Xow_wiki wiki, byte[] raw, int bgn, int len) {
		byte[] ttl_raw = ByteAry_.Mid(raw, bgn, len);
...
//				rv.Qarg_(ttl.Qarg_txt());
		...
		if (ttl.Anch_bgn() != ByteAry_.NotFound) rv.Anchor_(ttl.Anch_txt());
	}
	private static void Parse_site(Xoh_href rv, Url_encoder encoder, Xow_wiki wiki, byte[] raw, int bgn, int len) {	// /site/; EX: /site/fr.wikipedia.org/wiki/A
		int slash = Byte_ary_finder.Find_fwd(raw, Byte_ascii.Slash, bgn, len); if (slash == ByteAry_.NotFound) throw Err_mgr._.fmt_("xowa.href.parser", "invalid_site", "site href is missing slash: ~{0}", String_.new_utf8_(raw, bgn, len));
		rv.Tid_(Xoh_href.Tid_site);
		byte[] wiki_bry = ByteAry_.Mid(raw, bgn, slash);					// wiki is text between "/site/" and next "/"
		...
		byte[] page_bry = page_pos < len 
			? ByteAry_.Mid(raw, page_pos, len)								// page is text after next "/" + "/wiki/";
			: ByteAry_.Empty; 
		if (ByteAry_.Len_eq_0(page_bry))									// handle "/site/fr.wikipedia.org/wiki/"; note that these are generated by [[fr:]]
			page_bry = wiki.Props().Main_page();							// default to Main Page
//			int qarg_pos = Byte_ary_finder.Find_bwd(page_bry, Byte_ascii.Question);
//			byte[] qarg_bry = null;
//			if (qarg_pos != ByteAry_.NotFound) {
//				qarg_bry = ByteAry_.Mid(page_bry, qarg_pos + 1, page_bry.length);
//				rv.Qarg_(qarg_bry);
...
//			}
	...
	private static void Parse_ttl_and_resolve_xwiki(Gfo_usr_dlg usr_dlg, Xoh_href rv, Xow_wiki wiki, Url_encoder encoder, byte[] page_bry, byte[] raw, int bgn, int len) {
		...
		if (ttl.Wik_itm() != null) {				// page_bry has xwiki; EX: "wikt:A"; note that since this is called by "/site/", there may be two xwikis; EX: "w:wikt:"; Note that more than 2 is not being handled 
			...
			if (ByteAry_.Len_eq_0(ttl.Page_txt()))	// page_bry is just alias; EX: "wikt:"
		...
		if (ttl.Anch_bgn() != ByteAry_.NotFound)	// add anchor if it exists
