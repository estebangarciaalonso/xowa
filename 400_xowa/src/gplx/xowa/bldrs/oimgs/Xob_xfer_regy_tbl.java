...
class Xob_xfer_regy_tbl {
	...
	public static void Update(Db_stmt stmt, boolean done, int id, byte wkr_tid, String wkr_msg) {
		stmt.Clear()
		.Val_int_(id)
	...
	public static DataRdr Select(Db_provider p, byte repo_id, byte[] ttl, int limit) {
		Db_qry qry = Db_qry_.select_().Cols_all_()
			.From_(Tbl_name)
			.Where_(gplx.criterias.Criteria_.And(Db_crt_.mte_(Fld_oxr_xfer_repo, repo_id), Db_crt_.mt_(Fld_oxr_xfer_ttl, String_.new_utf8_(ttl))))
	...
	, Fld_oxr_file_is_orig = "oxr_file_is_orig", Fld_oxr_file_w = "oxr_file_w", Fld_oxr_xfer_thumbtime = "oxr_xfer_thumbtime"
	...
	, ", oxr_file_is_orig, oxr_file_w, oxr_xfer_thumbtime, oxr_xfer_count"
	, ", oxr_xfer_status, oxr_xfer_bin_tid, oxr_xfer_bin_msg"
	, ")"
	, "SELECT "
	, "  Min(oxt_lnki_id), oxt_orig_page_id, oxt_xfer_repo, oxt_xfer_ttl, oxt_xfer_ext, oxt_orig_media_type"
	, ", oxt_file_is_orig, oxt_file_w, oxt_xfer_thumbtime, Sum(oxt_xfer_count)"
	, ", 0, 0, ''"
	, "FROM    oimg_xfer_temp x"
	, "GROUP BY oxt_orig_page_id, oxt_xfer_repo, oxt_xfer_ttl, oxt_xfer_ext, oxt_orig_media_type, oxt_file_is_orig, oxt_file_w, oxt_xfer_thumbtime"
	);
