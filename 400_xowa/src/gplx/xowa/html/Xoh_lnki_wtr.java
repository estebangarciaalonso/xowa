...
public class Xoh_lnki_wtr {
	...
	public Xoh_lnki_wtr(Xoh_html_wtr html_wtr, Xow_wiki wiki, Xoh_html_wtr_cfg cfg) {
		...
		file_wtr = new Xoh_lnki_file_wtr(wiki, html_wtr);
	}
	public Xoh_lnki_file_wtr File_wtr() {return file_wtr;} private Xoh_lnki_file_wtr file_wtr;
	public void Init_by_ctx(Xop_ctx ctx) {
		this.ctx = ctx;
		this.page = ctx.Page();
		this.history_mgr = app.User().History_mgr();
		redlinks_mgr = ctx.Tab().Lnki_redlinks_mgr();
		file_wtr.Page_bgn(page);
	}
	public void Page_(Xoa_page v) {
		this.page = v;
		this.wiki = v.Wiki();
	}
	public void Write(ByteAryBfr bfr, Xoh_html_wtr_ctx hctx, byte[] src, Xop_lnki_tkn lnki) {
	...
	public void Write_plain_by_tkn(ByteAryBfr bfr, Xoh_html_wtr_ctx hctx, byte[] src, Xop_lnki_tkn lnki, Xoa_ttl lnki_ttl) {
		Write_plain(bfr, hctx, src, lnki, lnki_ttl, caption_tkn_wtr);
	}
	private void Write_plain(ByteAryBfr bfr, Xoh_html_wtr_ctx hctx, byte[] src, Xop_lnki_tkn lnki, Xoa_ttl lnki_ttl, Xop_lnki_caption_wtr caption_wkr) {
		...
		if (hctx.Mode_is_alt())
			...
			app.Href_parser().Build_to_bfr(lnki_ttl, wiki, bfr);	// '/wiki/A'
			if (cfg.Lnki_id()) {
	...
	private void Write_caption(ByteAryBfr bfr, Xop_ctx ctx, Xoh_html_wtr_ctx hctx, byte[] src, Xop_lnki_tkn lnki, byte[] ttl_bry, boolean tail_enabled, Xop_lnki_caption_wtr caption_wkr) {
	...
	private static boolean Write_caption_for_rel2abs(ByteAryBfr bfr, Xop_lnki_tkn lnki) {
	...
	private static final byte[] Bry_xowa_visited = ByteAry_.new_ascii_("\" class=\"xowa-visited"); 
}
interface Xop_lnki_caption_wtr {
	void Write_tkn(Xop_ctx ctx, Xoh_html_wtr_ctx hctx, ByteAryBfr bfr, byte[] src, Xop_tkn_grp grp, int sub_idx, Xop_tkn_itm tkn);
}
class Xop_lnki_caption_wtr_bry implements Xop_lnki_caption_wtr {
	...
	public void Write_tkn(Xop_ctx ctx, Xoh_html_wtr_ctx hctx, ByteAryBfr bfr, byte[] src, Xop_tkn_grp grp, int sub_idx, Xop_tkn_itm tkn) {
		bfr.Add(caption_bry);
	}
}
