...
public class Ref_itm_mgr {
	public Ref_itm_lst Lst_get(byte[] grp_name, int lst_idx) {
		return ByteAry_.Len_eq_0(grp_name) ? grp_default.Lsts_get_at(lst_idx) : ((Ref_itm_grp)grps.Fetch(grp_name)).Lsts_get_at(lst_idx);	// NOTE: must be ByteAry_.Len_eq_0 else <references group=""/> not same as <references/>; DATE:2013-02-06
	}	
	...
	Ref_itm_grp grp_default = new Ref_itm_grp(ByteAry_.Empty);
	int uid_last;		
}
