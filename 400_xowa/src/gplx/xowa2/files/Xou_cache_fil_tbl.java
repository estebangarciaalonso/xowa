...
class Xou_cache_fil_tbl {
	...
	public String Commit(Xou_cache_fil itm) {
		try {
			...
			switch (itm.Cmd_mode()) {
				case Db_cmd_mode.Tid_create:	stmt.Clear().Val_int(itm.Uid()); Commit_stmt_args(stmt, itm); stmt.Exec_insert(); break;
				case Db_cmd_mode.Tid_update:	stmt.Clear();					  Commit_stmt_args(stmt, itm); stmt.Val_int(itm.Uid()).Exec_update(); break;
				case Db_cmd_mode.Tid_delete:	stmt.Clear().Val_int(itm.Uid()); stmt.Exec_delete();	break;
