...
public class Xow_msg_mgr implements GfoInvkAble {
	...
	public byte[] Val_by_itm(ByteAryBfr tmp_bfr, Xol_msg_itm itm, Object[] args) {
		byte[] rv = itm.Val();
	...
	public byte[] Val_html_accesskey_and_title(byte[] id, ByteAryBfr bfr, Xowh_sidebar_itm itm) {
		byte[] tooltip_key = ByteAry_.Add(CONST_prefix_tooltip, id);
		byte[] tooltip_val = Val_by_key_obj(tooltip_key);
		boolean tooltip_found = ByteAry_.Len_gt_0(tooltip_val);
		byte[] accesskey_key = ByteAry_.Empty, accesskey_val = ByteAry_.Empty;
		boolean accesskey_found = false;
		if (tooltip_found) {
			accesskey_key = ByteAry_.Add(CONST_prefix_accesskey, id);
			accesskey_val = Val_by_key_obj(accesskey_key);
			accesskey_found = ByteAry_.Len_gt_0(accesskey_val);
		}
		if (accesskey_found)
			bfr.Add(CONST_atr_accesskey).Add(accesskey_val).Add_byte(Byte_ascii.Quote);
		bfr.Add(CONST_atr_title).Add(tooltip_found ? tooltip_val : ByteAry_.Empty);	// NOTE: if tooltip not found, make blank; don't bother showing tooltip_key
		if (accesskey_found)
	...
	}	static final byte[] CONST_prefix_tooltip = ByteAry_.new_ascii_("tooltip-"), CONST_prefix_accesskey = ByteAry_.new_ascii_("accesskey-"), CONST_atr_title = ByteAry_.new_ascii_(" title=\""), CONST_atr_accesskey = ByteAry_.new_ascii_(" accesskey=\"");
