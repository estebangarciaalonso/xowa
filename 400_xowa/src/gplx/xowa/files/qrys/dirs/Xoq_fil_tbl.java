...
//		}
	...
	public static void Insert(Db_provider p, byte[] ttl, byte status, byte orig_repo, byte[] orig_redirect, int orig_ext, int orig_w, int orig_h, String orig_url) {
		Db_stmt stmt = Db_stmt_.Null;
		try {
			stmt = Insert_stmt(p);
			Insert(stmt, ttl, status, orig_repo, orig_redirect, orig_ext, orig_w, orig_h, orig_url);
		}	finally {stmt.Rls();}
	}
	public static Db_stmt Insert_stmt(Db_provider p) {return Db_stmt_.new_insert_(p, Tbl_name, Fld_orig_ttl, Fld_status, Fld_orig_repo, Fld_orig_ext, Fld_orig_w, Fld_orig_h, Fld_orig_redirect, Fld_orig_url);}
	public static void Insert(Db_stmt stmt, byte[] ttl, byte status, byte orig_repo, byte[] orig_redirect, int orig_ext, int orig_w, int orig_h, String orig_url) {
		stmt.Clear()
		.Val_str_by_bry_(ttl)
		.Val_byte_(status)
		.Val_byte_(orig_repo)
	...
	,	", orig_url          varchar(1024)       NOT NULL"
	,	");"
	);
}
