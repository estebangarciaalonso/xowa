...
public class Xow_lang_mgr {
	...
	public ByteAryFmtr Html_div() {return html_div;} ByteAryFmtr html_div = ByteAryFmtr.new_(
		String_.Concat_lines_nl_skipLast
		(	"<div id=\"xowa-lang\">"
		,	"  <h5>~{langs_hdr}~{wikidata_link}</h5>~{grps}"
		,	"</div>"
		), "grps", "langs_hdr", "wikidata_link");
	public ByteAryFmtr Html_wikidata_link() {return html_wikidata_link;} ByteAryFmtr html_wikidata_link = ByteAryFmtr.new_(" (<a href=\"/site/www.wikidata.org/wiki/~{qid}\">wikidata</a>)", "qid");
	...
	public void Html_bld(ByteAryBfr bfr, Xow_wiki wiki, ListAdp ttl_list, byte[] qid) {
...
class Xow_lang_html implements ByteAryFmtrArg {
	int stage = 0;
	...
	public void XferAry(ByteAryBfr bfr, int idx) {
		switch (stage) {
			case 0: {
				...
				byte[] wikidata_link = ByteAry_.Len_eq_0(qid) ? ByteAry_.Empty : lang_mgr.Html_wikidata_link().Bld_bry_many(tmp_bfr, qid);
