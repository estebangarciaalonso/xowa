...
class Xou_db_xtn_tbl {
	...
	public void Db_save(Xou_db_xtn_itm itm) {
		...
		switch (itm.Cmd_mode()) {
			case Db_cmd_mode.Tid_create:	stmt.Clear().Val_str(itm.Key()).Val_str(itm.Version()).Exec_insert(); break;
			case Db_cmd_mode.Tid_update:	stmt.Clear()					.Val_str(itm.Version()).Val_str(itm.Key()).Exec_update(); break;
			case Db_cmd_mode.Tid_delete:	stmt.Clear().Val_str(itm.Key()).Exec_delete();	break;
