...
public class Xodb_load_mgr_txt implements Xodb_load_mgr {
	...
	public void Load_search(Cancelable cancelable, ListAdp rv, byte[] search, int results_max) {
		...
		if (search_len > 0 && search[search_len - 1] == Byte_ascii.Asterisk) {
			search = ByteAry_.Mid(search, 0, search_len - 1);
		...
		if (match_tid == Xosrh_core.Match_tid_all) {
			...
			byte[] end_ttl = ByteAry_.Increment_last(ByteAry_.Copy(search));
...
//				if (match.Itms_len() == max_results) break;
	...
	private void Load_ctg_v2_data_mgrs(Gfo_usr_dlg usr_dlg, byte[] ctg_name, byte[] src, int bgn, int[] grp_lens, Xoctg_idx_mgr[] grp_mgrs) {
		for (int i = 0; i < 3; i++) {
			...
			byte[] grp_src = ByteAry_.Mid(src, bgn, end);
	...
	private static void Load_ctg_v1_parse(ListAdp rv, Gfo_usr_dlg usr_dlg, byte[] ary) {
		...
		for (int i = 0; i < rowCount; i++) {
			...
			rv.Add(row);
			if (garbage) continue;
			int bgn = pos + 1 + (i * (Base85_utl.Len_int + gplx.xowa.apps.Apps_app_mgr.Len_dlm_fld));
			try {
...
//					rslt_list_ttls[rslt_list_len++] = ttl_itm;
	...
	public byte[] Load_qid(byte[] wiki_alias, byte[] ns_num, byte[] ttl) {
		...
		return ByteAry_.Mid(qid_itm.Src(), qid_itm.Itm_bgn() + ttl.length + 1, qid_itm.Itm_end());	// extract qid; note that all itms have format of "ttl|qid"
	}	Io_url qids_root;
	public int Load_pid(byte[] lang_key, byte[] pid_name) {
		...
		return ByteAry_.XtoIntByPos(pid_itm.Src(), pid_itm.Itm_bgn() + pid_name.length + 1 + 1, pid_itm.Itm_end(), gplx.xowa.xtns.wdatas.Wdata_wiki_mgr.Pid_null);	// extract pid; note that all itms have format of "ttl|pid"; +1=skip pipe; +1 skip p
	}	Io_url pids_root;
	...
	public Xodb_page[] Load_ctg_list(byte[][] ttls) {
		...
		Xoctg_data_ctg ctg_temp = new Xoctg_data_ctg(ByteAry_.Empty);
		for (int i = 0; i < len; i++) {
