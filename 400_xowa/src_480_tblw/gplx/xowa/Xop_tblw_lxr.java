	...
	public int Make_tkn(Xop_ctx ctx, Xop_tkn_mkr tkn_mkr, Xop_root_tkn root, byte[] src, int src_len, int bgn_pos, int cur_pos) {
		...
		if (	last_tkn != null
			...
			if (	lnki.Pipe_count_is_zero()) {
				...
				switch (wlxr_type) {	// "||" found; check if in lnki and validate ttl; DATE:2014-03-29
					...
					case Xop_tblw_wkr.Tblw_type_td2:	// ||
						invalidate = !Xop_lnki_wkr_.Parse_ttl(ctx, src, lnki, bgn_pos);	// only invalidate if invalid ttl
						break;
				}
				if (invalidate) {
					ctx.Stack_pop_last();
