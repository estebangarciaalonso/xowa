...
class Xofw_wiki_wkr_mock implements Xofw_wiki_finder {
	...
	Xofw_wiki_wkr_mock Clear(int repo_idx, byte[] repo_wiki_key) {
		...
		if_ttl = then_redirect = ByteAry_.Empty;
		return this;
	}	static final byte[] Bry_commons = ByteAry_.new_ascii_("commons.wikimedia.org"), Bry_en_wiki = ByteAry_.new_ascii_("en.wikipedia.org");
	...
	public boolean Locate(Xofw_file_finder_rslt rv, ListAdp repo_pairs, byte[] ttl) {
		rv.Init(ttl);
		byte[] redirect = ByteAry_.Eq(ttl, if_ttl) ? then_redirect : null;
...
class Xofw_wiki_wkr_base implements Xofw_wiki_finder {
	...
	public void Find(ListAdp repo_pairs, Xof_xfer_itm file) {
		...
		for (int i = 0; i < repo_pairs_len; i++) {
			...
			boolean found = repo_wiki.Db_mgr().Load_mgr().Load_ttl(tmp_db_page, file_ns, ttl.Page_db());
			if (!found) {continue;}
	...
	public boolean Locate(Xofw_file_finder_rslt rv, ListAdp repo_pairs, byte[] ttl_bry) {
		...
		for (int i = 0; i < repo_pairs_len; i++) {
			...
			boolean found = src_wiki.Db_mgr().Load_mgr().Load_ttl(tmp_db_page, file_ns, ttl_db_key);
