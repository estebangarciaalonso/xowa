...
import gplx.xowa.bldrs.*; import gplx.xowa.wikis.caches.*; import gplx.xowa.parsers.lnkis.*;
...
//			Parse_ns(wiki.Ns_mgr().Get_by_id(Xow_ns_.Id_help));
		...
		ConsoleAdp._.WriteLine(Int_.XtoStr(Pf_xtn_ifexist.Count));
	}
	public void Cmd_end() {
		img_dump.Wkr_end();
		math_dump.Wkr_end();
...
//			Tfds.Write(String_.new_utf8_(ttl_bry));
		...
		try {
			msg_log.Clear();
			if (ns_is_tmpl)
				parser.Parse_tmpl(ctx, ctx.Tkn_mkr(), wiki.Ns_mgr().Ns_template(), ttl_bry, src);
			else {
