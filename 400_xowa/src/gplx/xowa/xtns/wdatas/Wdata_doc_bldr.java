...
public class Wdata_doc_bldr {
	...
	public Wdata_doc_bldr Qid_(String v) {this.qid = ByteAry_.new_ascii_(v); return this;} private byte[] qid; ListAdp props = ListAdp_.new_();
	...
	}	private static ByteAryBfr tmp_bfr = ByteAryBfr.new_(); private static byte[] Bry_year_prefix = ByteAry_.new_ascii_("+0000000");
	...
	private void Xto_bry__aliases() {
		...
		for (int i = 0; i < len; i++) {
			...
			wtr.Key(i != 0, ByteAry_.new_utf8_(alias.Lang()));
			wtr.Ary_bgn();
			...
			for (int j = 0; j < aliases_len; j++) {
				...
				wtr.Val(j != 0, ByteAry_.new_utf8_(aliases_str));
			}
			wtr.Ary_end();
		}
		wtr.Nde_end();
