...
//				rv.Qarg_(ttl.Qarg_txt());
	...
	private static void Parse_site(Xoh_href rv, Url_encoder encoder, Xow_wiki wiki, byte[] raw, int bgn, int len) {	// /site/; EX: /site/fr.wikipedia.org/wiki/A
		int slash = ByteAry_.FindFwd(raw, Byte_ascii.Slash, bgn, len); if (slash == ByteAry_.NotFound) throw Err_mgr._.fmt_("xowa.href.parser", "invalid_site", "site href is missing slash: ~{0}", String_.new_utf8_(raw, bgn, len));
		rv.Tid_(Xoh_href.Tid_site);
...
//			int qarg_pos = ByteAry_.FindBwd(page_bry, Byte_ascii.Question);
//			byte[] qarg_bry = null;
