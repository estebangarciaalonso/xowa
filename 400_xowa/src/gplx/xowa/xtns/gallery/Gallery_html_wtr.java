...
public class Gallery_html_wtr {
	...
	public void Write_html(Xoa_app app, Xow_wiki wiki, Xop_ctx ctx, Xoa_page page, Xoh_html_wtr wtr, Xoh_html_wtr_ctx hctx, ByteAryBfr bfr, byte[] src, Gallery_xnde mgr) {
		...
		ByteAryBfr itm_bfr = wiki.Utl_bry_bfr_mkr().Get_k004();
		int mgr_elem_id = -1;
		for (int i = 0; i < itms_len; i++) {
			...
			if (	itm_ttl != null				// ttl does not have invalid characters
				...
				byte[] lnki_alt = itm.Alt_bgn() == ByteAry_.NotFound ? lnki_ttl : Xoh_html_wtr_escaper.Escape(app, tmp_bfr, ByteAry_.Mid(src, itm.Alt_bgn(), itm.Alt_end())); 
	...
	private static final byte[] Gallery_box_cls_packed = ByteAry_.new_utf8_(" mw-gallery-packed");
	private ByteAryFmtr
	  fmtr_mgr_box = ByteAryFmtr.new_(String_.Concat_lines_nl_skipLast
	...
	, fmtr_gallery_itm_img = ByteAryFmtr.new_(String_.Concat_lines_nl_skipLast
	(	""
	...
	, fmtr_gallery_itm_txt = ByteAryFmtr.new_(String_.Concat_lines_nl_skipLast
	(	""
	...
	, fmtr_mgr_box_style_none		= ByteAryFmtr.new_()
	, fmtr_mgr_box_style_per_row	= ByteAryFmtr.new_("max-width:~{gallery_width}px; _width:~{gallery_width}px;", "gallery_width")
	;
}
