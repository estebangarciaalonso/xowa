...
public class Xoi_cmd_mgr implements GfoInvkAble {
	...
	void Process_async(Gfo_thread_cmd cmd) {
	...
	void Run_async(Gfo_thread_cmd cmd) {ThreadAdp_.invk_msg_(this, GfoMsg_.new_cast_(Invk_process_async).Add("v", cmd)).Start();}
	void Cmds_run() {
	...
	Gfo_thread_cmd Dump_cmd_new(String wiki_key, String wiki_date, String dump_type, String cmd_key) {
		if		(String_.Eq(cmd_key, Xoi_cmd_wiki_download.KEY_dump))					return new Xoi_cmd_wiki_download().Ctor_download_(install_mgr, wiki_key, wiki_date, dump_type).Owner_(this);
		...
		else if	(String_.Eq(cmd_key, Xoi_cmd_category2_download_page_props.KEY_category2))		return new Xoi_cmd_category2_download_page_props(install_mgr, wiki_key, wiki_date).Owner_(this);
		else if	(String_.Eq(cmd_key, Xoi_cmd_category2_download_categorylinks.KEY_category2))	return new Xoi_cmd_category2_download_categorylinks(install_mgr, wiki_key, wiki_date).Owner_(this);
		else																			throw Err_mgr._.unhandled_(cmd_key);
	}
	public static final String[] Wiki_cmds_valid = new String[] {Xoi_cmd_wiki_download.KEY_dump, Xoi_cmd_wiki_unzip.KEY_dump, Xoi_cmd_wiki_import.KEY, Xoi_cmd_wiki_zip.KEY, Xoi_cmd_category2_build.KEY, Xoi_cmd_category2_download_page_props.KEY_category2, Xoi_cmd_category2_download_categorylinks.KEY_category2};
	...
	Gfo_thread_cmd Cmd_new(GfoMsg m) {
		...
		if		(String_.Eq(cmd_key, Gfo_thread_cmd_download.KEY))						return new Gfo_thread_cmd_download().Init("downloading", m.ReadStr("src"), ByteAryFmtr_eval_mgr_.Eval_url(app.Url_cmd_eval(), m.ReadBry("trg"))).Url_eval_mgr_(app.Url_cmd_eval()).Owner_(this).Ctor(app.Usr_dlg(), app.Gui_mgr().Kit());
		else if	(String_.Eq(cmd_key, Gfo_thread_cmd_unzip.KEY))							return new Gfo_thread_cmd_unzip().Url_eval_mgr_(app.Url_cmd_eval()).Owner_(this).Init(app.Usr_dlg(), app.Gui_mgr().Kit(), app.Fsys_mgr().App_mgr().App_decompress_bz2(), app.Fsys_mgr().App_mgr().App_decompress_zip(), ByteAryFmtr_eval_mgr_.Eval_url(app.Url_cmd_eval(), m.ReadBry("src")), ByteAryFmtr_eval_mgr_.Eval_url(app.Url_cmd_eval(), m.ReadBry("trg")));
		else if	(String_.Eq(cmd_key, Gfo_thread_cmd_replace.KEY))						return new Gfo_thread_cmd_replace().Url_eval_mgr_(app.Url_cmd_eval()).Owner_(this).Init(app.Usr_dlg(), app.Gui_mgr().Kit(), ByteAryFmtr_eval_mgr_.Eval_url(app.Url_cmd_eval(), m.ReadBry("fil")));
		else if	(String_.Eq(cmd_key, Xoi_cmd_wiki_image_cfg.KEY_dump))					return new Xoi_cmd_wiki_image_cfg(app, ByteAryFmtr_eval_mgr_.Eval_url(app.Url_cmd_eval(), m.ReadBry("fil"))).Owner_(this);
...
//			else if	(String_.Eq(cmd_key, Gfo_thread_exec_sync.KEY))							return new Gfo_thread_exec_sync(app.Usr_dlg(), app.Gui_mgr().Kit(), ByteAryFmtr_eval_mgr_.Eval_url(app.Url_cmd_eval(), m.ReadBry("url")), m.ReadStr("args")).Owner_(this);
		else if	(String_.Eq(cmd_key, Xoi_cmd_imageMagick_download.KEY_imageMagick))		return new Xoi_cmd_imageMagick_download(app.Usr_dlg(), app.Gui_mgr().Kit(), ByteAryFmtr_eval_mgr_.Eval_url(app.Url_cmd_eval(), m.ReadBry("trg"))).Owner_(this);
