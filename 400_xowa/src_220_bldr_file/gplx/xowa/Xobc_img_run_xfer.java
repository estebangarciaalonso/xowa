...
import gplx.ios.*; import gplx.gfui.*; import gplx.xowa.bldrs.*; 
...
//			Io_mgr._.DeleteDirDeep_ary(errors_dir);
	...
	}	Io_url tmp_dir; ByteAryBfr url_bfr = ByteAryBfr.new_(); Xof_file_stat[] ext_stats; int fail_count; Xof_file_mgr file_mgr;
	byte[] Redirect(Xofo_file row) {return row.Redirect_exists() ? row.Name() : ByteAry_.Empty;}	// handle redirect rows which are reversed (Redirect|Name)
		...
		while (rdr.Read_next()) {
			try {
				...
				for (int i = 0; i < lnks_len; i++) {
					...
					xfer_itm.Atrs_by_lnki(lnk.Lnki_type(), lnk.Lnki_w(), lnk.Lnki_h(), lnk.Lnki_upright(), lnk.Lnki_thumbtime());
...
//						xfer.Regy_itm().Mode_set(xfer_itm.File_type().Id_is_video() ? Xof_meta_itm.Mode_thumb_missing : Xof_meta_itm.Mode_orig_missing);
					row.Status_msg_(ByteAry_.new_utf8_(xfer.Rslt().Err_msg()));
...
//			}
	...
	ByteAryBfr dump_bfr = ByteAryBfr.new_(); Gfo_fld_wtr fld_wtr; Gfo_fld_rdr fld_rdr; ByteAryBfr tmp = ByteAryBfr.new_();
