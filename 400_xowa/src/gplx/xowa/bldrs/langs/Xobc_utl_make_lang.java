...
public class Xobc_utl_make_lang implements GfoInvkAble {
	...
	public void Bld_all() {
		...
		kwd_mgr.Add_words();
		lang_parser.Save_langs(app, lang_root.GenSubDir(Xol_mw_lang_parser.Dir_name_xowa), manual_text_hash);
		app.Usr_dlg().Prog_many("", "", "done");
	}
	public Object Invk(GfsCtx ctx, int ikey, String k, GfoMsg m) {
		if		(ctx.Match(k, Invk_kwds))					return kwd_mgr;
		else if	(ctx.Match(k, Invk_build_all))				Bld_all();
		else if	(ctx.Match(k, Invk_manual_text))			Parse_manual_text(m.ReadBry("langs"), m.ReadBry("text"));
		else												return GfoInvkAble_.Rv_unhandled;
		return this;
	}	private static final String Invk_kwds = "keywords", Invk_manual_text = "manual_text", Invk_build_all = "build_all";
	public void Parse_manual_text(byte[] langs_bry, byte[] text) {
