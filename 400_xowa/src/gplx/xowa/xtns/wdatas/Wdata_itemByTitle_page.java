...
public class Wdata_itemByTitle_page implements Xows_page {
	...
	private static final byte[] Arg_site = ByteAry_.new_ascii_("site"), Arg_page = ByteAry_.new_ascii_("page");
	public ByteAryFmtr Html_fmtr() {return html_fmtr;}
	...
	public void Special_gen(Xoa_url calling_url, Xoa_page page, Xow_wiki wiki, Xoa_ttl ttl) {
		...
		if (site_bgn != ByteAry_.NotFound) {						// leaf arg is available
			...
			if (page_bgn != ByteAry_.NotFound && page_bgn < raw_bry_len) {	// pipe is found and not last char (EX: "enwiki/" is invalid
				...
				page_bry = ByteAry_.Mid(raw_bry, page_bgn + 1, raw_bry_len);
			}
		}
		Xoa_app app = wiki.App();
		if (ByteAry_.Len_gt_0(site_bry) && ByteAry_.Len_gt_0(page_bry))
