...
public class Xof_offline_png_tst {
	...
	@Test   public void Copy_thumb() {
		...
		fxt.Test_get(Xof_fsdb_arg_exec_get.new_().Init_thumb("A.png").Rslt_reg_missing_reg().Rslt_qry_mock_().Rslt_bin_fsdb_());
		fxt.Test_fsys("mem/root/common/thumb/7/0/A.png/220px.png", "220,200");
		fxt.Test_regy(fxt.Key_bldr().Bld_as_str("A.png", Xop_lnki_type.Id_thumb, 220), "common/thumb/7/0/A.png/220px.png");
		fxt.Test_get(Xof_fsdb_arg_exec_get.new_().Init_thumb("A.png").Rslt_reg_found_url());	// get it again; make sure url is found in reg
	}
	@Test   public void Copy_orig() {
		if (Xo_test.Db_skip) return;
		fxt.Init_qry_xowa(Xof_fsdb_arg_init_qry.new_().Init_commons("A.png", 440, 400));
		fxt.Init_bin_fsdb(Xof_fsdb_arg_init_bin.new_().Init_commons_orig("A.png", 440, 400));
		fxt.Test_get(Xof_fsdb_arg_exec_get.new_().Init_orig("A.png").Rslt_reg_missing_reg().Rslt_qry_mock_().Rslt_bin_fsdb_());
		fxt.Test_fsys("mem/root/common/orig/7/0/A.png", "440,400");
		fxt.Test_regy_pass(fxt.Key_bldr().Bld_as_str("A.png"));
	}
	@Test   public void Make_thumb() {
		if (Xo_test.Db_skip) return;
		fxt.Init_qry_xowa__bin_fsdb__commons_orig("A.png", 440, 400);
		fxt.Test_get(Xof_fsdb_arg_exec_get.new_().Init_thumb("A.png").Rslt_reg_missing_reg().Rslt_qry_mock_().Rslt_bin_fsdb_().Rslt_cnv_y_());
		fxt.Test_fsys("mem/root/common/thumb/7/0/A.png/220px.png", "220,200");
		fxt.Test_regy_pass(fxt.Key_bldr().Bld_as_str("A.png", Xop_lnki_type.Id_thumb, 220));
	}
}
