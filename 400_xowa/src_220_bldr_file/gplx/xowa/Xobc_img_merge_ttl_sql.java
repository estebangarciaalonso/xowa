...
public class Xobc_img_merge_ttl_sql extends Xob_itm_dump_base implements Xob_cmd, GfoInvkAble {
	public Xobc_img_merge_ttl_sql(Xob_bldr bldr, Xow_wiki wiki) {this.Cmd_init(bldr, wiki); this.make_fil_len = Io_mgr.Len_mb;}
	...
	void Merge(boolean pass_0, Io_line_rdr name_rdr, Io_line_rdr size_rdr) {
		while (name_rdr.Read_next()) {
			...
			byte[] name_lower = Xof_ext_.Lower_ext(ByteAry_.Copy(itm.Name()));
	...
	ByteAryBfr noop = ByteAryBfr.new_();
}
