...
public class Xof_tbl_reg {
	...
	public static void Search(Db_provider p, OrderedHash itms, int itms_len) {
		for (int i = 0; i < itms_len; i += Max_rows_per_rdr) {
			...
			Search_by_grp(p, itms, keys);
		}
	}
	private static void Search_by_grp(Db_provider p, OrderedHash itms, Object[] keys) {
		...
		try {
			...
			while (rdr.MoveNextPeer()) {
				...
				switch (status) {
					case Xof_fsdb_itm.Reg_status_present:	itm.Reg_url_(rdr.ReadStr(Fld_reg_url)); break;	// itm present; set url
					case Xof_fsdb_itm.Reg_status_deleted:	break;											// itm deleted; deleted by cache cleaner or never created locally
					case Xof_fsdb_itm.Reg_status_unknown:	break;											// itm unknown; not in fsdb and not in wmf; ignore; page will just show href
