...
import gplx.dbs.*; import gplx.xowa.files.fsdb.*; import gplx.fsdb.meta.*;
...
public class Xow_file_mgr implements GfoInvkAble {
	...
	public Xow_file_mgr(Xow_wiki wiki) {
		...
		file_meta_wkr_as_db_orig.Tbl().Init_by_db(Bool_.Y).Conn_(Db_conn_pool.I.Get_or_new__sqlite(wiki.Fsys_mgr().File_dir().GenSubFil_nest("wiki.orig#00.sqlite3")));
	...
	public void Cfg_set(String grp, String key, String val) {	// TEST: should only be called by tests
		...
		Fsm_cfg_grp grp_itm = (Fsm_cfg_grp)test_grps.Fetch(grp);
		if (grp_itm == null) {
			grp_itm = new Fsm_cfg_grp(grp);
			test_grps.Add(grp, grp_itm);
		}
		grp_itm.Upsert(key, val);
	}	private HashAdp test_grps;
	public Fsm_cfg_grp Cfg_get(String grp) {
		if (test_grps != null) {
			Fsm_cfg_grp rv = (Fsm_cfg_grp)test_grps.Fetch(grp);
			return rv == null ? Fsm_cfg_grp.Null : rv;
		}
		if (this.Version() == Version_1) return Fsm_cfg_grp.Null;
