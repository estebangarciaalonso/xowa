...
class Xot_tmpl_wtr {
	public byte[] Write_all(Xop_ctx ctx, Xop_root_tkn root, byte[] src) {
		ByteAryBfr trg = ctx.App().Utl_bry_bfr_mkr().Get_m001();
	...
	private void Write_tkn(Xop_ctx ctx, byte[] src, int src_len, ByteAryBfr trg, Xop_tkn_itm tkn) {
		switch (tkn.Tkn_tid()) {
			...
			case Xop_tkn_itm_.Tid_tvar:
				...
				trg.Add_mid(src, tvar_tkn.Txt_bgn(), tvar_tkn.Txt_end());
				break;
	...
	.Add(":", ByteAry_.new_utf8_("&#x3A;")).Add("_", ByteAry_.new_utf8_("&#95;")).Add("*", ByteAry_.new_utf8_("&#42;")).Add(" ", ByteAry_.new_utf8_("&#32;"))
	;
}
