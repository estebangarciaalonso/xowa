...
class Xoctg_url_mok extends Xoctg_url {	public Xoctg_url_mok Page_bgn_(String v) {return Grp(Xoa_ctg_mgr.Tid_page, Bool_.Y, v);}
	...
	Xoctg_url_mok Grp(byte tid, boolean v, String bmk) {
		...
		this.Grp_idxs()[tid] = ByteAry_.new_ascii_(bmk);
		return this;
	}	
}
class Xodb_load_mgr_sql_fxt {
	...
	}	Db_mgr_fxt fxt; IntRef next_id = IntRef.new_(1); Xoa_app app; Xow_wiki wiki;
...
class Xoctg_mok_ctg {
	public Xoctg_mok_ctg(IntRef next_id) {this.next_id = next_id;} IntRef next_id;
	public byte[] Ttl() {return ttl;}
	public Xoctg_mok_ctg Ttl_(String v) {return Ttl_(ByteAry_.new_ascii_(v));}
	...
	public Xoctg_mok_ctg Grp_pages_(int bgn, int end, String last_itm_plus_one_sortkey) {
		...
		byte[] ttl_prefix = ByteAry_.new_ascii_("Page_");
		int ns_id = Xow_ns_.Id_main;
		byte ctg_tid = Xoa_ctg_mgr.Tid_page;
		for (int i = bgn; i < end; i++) {
			byte[] ttl = ByteAry_.Add(ttl_prefix, ByteAry_.new_ascii_(Int_.XtoStr_PadBgn(i, 3)));
		...
		grp.Last_plus_one_sortkey_(ByteAry_.new_ascii_(last_itm_plus_one_sortkey));
		return this;
	}
}
