...
class Pf_xtn_ifexist extends Pf_func_base {
	...
	public static boolean Exists(Xodb_page rv, Xop_ctx ctx, byte[] val_dat_ary) {
		...
		if (exists_val != null) {
			boolean exists = ((BoolVal)exists_val).Val();
			rv.Exists_(exists);
			return exists;
		}
	...
	@Override public Pf_func New(int id, byte[] name) {return new Pf_xtn_ifexist().Name_(name);}
}	
