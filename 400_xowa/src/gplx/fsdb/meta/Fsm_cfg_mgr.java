...
public class Fsm_cfg_mgr {		
	...
	public Fsm_cfg_grp Grps_get_or_load(String grp_key) {
		Fsm_cfg_grp grp = (Fsm_cfg_grp)grps.Fetch(grp_key);
		if (grp == null) {
	...
	public Fsm_cfg_grp Grps_get_or_add(String grp_key) {	// TEST:
		Fsm_cfg_grp grp = (Fsm_cfg_grp)grps.Fetch(grp_key);
		if (grp == null) {
			grp = new Fsm_cfg_grp(grp_key);
			grps.Add(grp_key, grp);
		}
		return grp;
	}
	public static Fsm_cfg_mgr load_(Fsm_abc_mgr abc_mgr, Db_conn p) {return new Fsm_cfg_mgr().Init_by_load(p);}
	public static Fsm_cfg_mgr make_(Fsm_abc_mgr abc_mgr, Db_conn p) {return new Fsm_cfg_mgr().Init_by_make(p);}
	private Fsm_cfg_mgr Init_by_load(Db_conn p) {
		this.cfg_tbl = new Fsm_cfg_tbl().Conn_(p, 0, true, false);
		Fsm_cfg_grp core_grp = Grps_get_or_load(Grp_core);
	...
	private Fsm_cfg_mgr Init_by_make(Db_conn p) {
		this.cfg_tbl = new Fsm_cfg_tbl().Conn_(p, 0, true, true);
