...
class Cache_dir_tbl {
	...
	public String Db_save(Cache_dir_itm itm) {
		try {
			...
			switch (itm.Cmd_mode()) {
				case Db_cmd_mode.Tid_create:	stmt.Clear().Val_int(itm.Uid()).Val_bry_as_str(itm.Dir_bry()).Exec_insert(); break;
				case Db_cmd_mode.Tid_update:	stmt.Clear()					.Val_bry_as_str(itm.Dir_bry()).Val_int(itm.Uid()).Exec_update(); break;
				case Db_cmd_mode.Tid_delete:	stmt.Clear().Val_int(itm.Uid()).Exec_delete();	break;
