...
*/
package gplx.xowa.gui.wins.url_macros; import gplx.*; import gplx.xowa.*; import gplx.xowa.gui.*; import gplx.xowa.gui.wins.*;
public class Xog_url_macro_mgr {
	private ByteAryBfr bfr = ByteAryBfr.reset_(255);
	public Xog_url_macro_mgr() {
		this.Init();
	}
	public void Lang_default_(byte[] v) {lang_default = v;} private byte[] lang_default = ByteAry_.new_ascii_("en");
	...
	private byte[] Fmt_or_null__type(byte[] raw, int raw_len, int dot_pos, int colon_pos) {
		...
		Object custom_obj = custom_mgr.Trie().MatchAtCurExact(raw, type_bgn, type_end);
		if (custom_obj == null) {
			...
			byte[] lang_bry = dot_missing ? lang_default : ByteAry_.Mid(raw, 0, dot_pos);
			Xog_url_macro_itm type_itm = (Xog_url_macro_itm)type_obj;
			return type_itm.Fmtr_exec(bfr, lang_bry, ByteAry_.Mid(raw, colon_pos + 1, raw_len));
		}
		else {
			Xog_url_macro_itm custom_itm = (Xog_url_macro_itm)custom_obj;
			return custom_itm.Fmtr_exec(bfr, ByteAry_.Mid(raw, colon_pos + 1, raw_len));
		}
	}
	private void Init() {
