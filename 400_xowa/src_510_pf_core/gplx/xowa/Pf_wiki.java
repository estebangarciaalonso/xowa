...
class Pf_wiki_props extends Pf_func_base {
	@Override public void Func_evaluate(Xop_ctx ctx, byte[] src, Xot_invk caller, Xot_invk self, ByteAryBfr bb) {
	    ...
	    switch (id) {
			...
			case Xol_kwd_grp_.Id_site_contentlanguage: bb.Add(ctx.Page().Lang().Key_bry()); break;
	...
	@Override public void Func_evaluate(Xop_ctx ctx, byte[] src, Xot_invk caller, Xot_invk self, ByteAryBfr bb) {
		...
		switch (id) {
			...
			case Xol_kwd_grp_.Id_rev_pagesize:
				...
				break;
			case Xol_kwd_grp_.Id_rev_protectionlevel: bb.Add(props.ProtectionLevel()); break;
			default: throw Err_.unhandled(id);
		}
	}
