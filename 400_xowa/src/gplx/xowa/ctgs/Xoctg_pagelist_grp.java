...
public class Xoctg_pagelist_grp implements ByteAryFmtrArg {
	public void Init_app(Xoa_app app, boolean type_is_normal, ByteAryFmtr fmtr_grp, ByteAryFmtr fmtr_itm) {
		this.type_is_normal = type_is_normal;
		this.fmtr_grp = fmtr_grp;
		itms.Init_app(app, fmtr_itm);
	}	private ByteAryFmtr fmtr_grp;
	public void Init_by_wiki(Xow_wiki wiki) {
		lbl_ctg_text	= wiki.Msg_mgr().Val_by_id(Xol_msg_itm_.Id_ctg_tbl_hdr);
		lbl_ctg_help	= Pf_msg_mgr.Get_msg_by_key(wiki, wiki.Lang(), Key_pagecategorieslink, ByteAry_.Ary_empty);
		lbl_hidden		= wiki.Msg_mgr().Val_by_id(Xol_msg_itm_.Id_ctg_tbl_hidden);
		itms.Init_wiki(wiki);
	}	private byte[] lbl_ctg_help, lbl_ctg_text, lbl_hidden; private static final byte[] Key_pagecategorieslink = ByteAry_.new_ascii_("pagecategorieslink");
