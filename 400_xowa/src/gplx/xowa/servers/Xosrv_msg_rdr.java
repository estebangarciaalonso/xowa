...
public class Xosrv_msg_rdr {
	...
	public Xosrv_msg Read() {
		...
		int body_len = ByteAry_.XtoIntByPos(header_bry,  2, 12, -1); 	if (body_len == -1)					return Xosrv_msg.fail_("body_len is not number; body_len:{0}", String_.new_utf8_(header_bry,  2, 23));
		int cksum    = ByteAry_.XtoIntByPos(header_bry, 13, 23, -1);	if (cksum == -1)					return Xosrv_msg.fail_("checksum is not number; cksum:{0}", String_.new_utf8_(header_bry, 13, 23));
