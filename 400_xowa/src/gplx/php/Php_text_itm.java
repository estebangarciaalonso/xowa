...
interface Php_text_itm {
	...
	int Src_bgn();
	int Src_end();
	void Bld(ByteAryBfr bfr, byte[] src);
}
class Php_text_itm_ {
...
class Php_text_itm_text implements Php_text_itm {
	...
	public void Bld(ByteAryBfr bfr, byte[] src) {bfr.Add_mid(src, src_bgn, src_end);}
}
class Php_text_itm_escaped implements Php_text_itm {
	...
	public void Bld(ByteAryBfr bfr, byte[] src) {bfr.Add_byte(literal);}
}
class Php_text_itm_utf16 implements Php_text_itm {
	...
	public int Src_end() {return src_end;} private int src_end;
	public byte[] Literal() {return literal;} private byte[] literal;
	public void Bld(ByteAryBfr bfr, byte[] src) {bfr.Add(literal);}
}
class Php_text_itm_arg implements Php_text_itm {
	...
	public void Bld(ByteAryBfr bfr, byte[] src) {
