...
public class Xop_list_wkr implements Xop_ctx_wkr {
	...
	public int MakeTkn_bgn(Xop_ctx ctx, Xop_tkn_mkr tkn_mkr, Xop_root_tkn root, byte[] src, int src_len, int bgn_pos, int cur_pos) {// REF.MW: Parser|doBlockLevels
		...
		if (SymAry_fill_overflow) return ctx.LxrMake_txt_(cur_pos);
		...
		if (allDd && cur_pos < src_len - 2 && src[cur_pos] == '{' && src[cur_pos + 1] == '|') // NOTE: if indent && next == {| then invoke table; EX: ":::{|"
			return ctx.Tblw().MakeTkn_bgn(ctx, tkn_mkr, root, src, src_len, cur_pos, cur_pos + 2, Xop_tblw_wkr.Tblw_type_tb, true, false, -1, -1);	// NOTE: ws_enabled must be set to true; see test for Adinkras; Cato the Elder
		else {
	...
	int SymAry_fill(byte[] src, int cur_pos, int src_len, byte curByt) {
		curSymLen = 0;
