...
public class Xoh_rtl_utl {		
	...
	private static ByteAryBfr bfr = ByteAryBfr.reset_(32);
	public static byte[] Reverse_li(byte[] src) {
		int src_len = src.length;
		int pos = 0;
		while (true) {
			int ul_bgn = Byte_ary_finder.Find_fwd(src, Ul_bgn, pos, src_len);
			if (ul_bgn == Byte_ary_finder.Not_found) break;	// no more <ul
			int ul_end = Byte_ary_finder.Find_fwd(src, Ul_end, ul_bgn + Ul_bgn.length, src_len);
			if (ul_end == Byte_ary_finder.Not_found) break;	// no more </ul>
	...
	private static void Reverse_li_src(ByteAryBfr bfr, byte[] src, int ul_bgn, int ul_end, int[] tmp_ary) {
		int pos = ul_bgn;
		int tmp_idx = 0;
		while (true) {
			int li_bgn = Byte_ary_finder.Find_fwd(src, Li_bgn, pos, ul_end);
			if (li_bgn == Byte_ary_finder.Not_found) break;	// no more <li
			int li_end = Byte_ary_finder.Find_fwd(src, Li_end, pos, ul_end);
			if (li_end == Byte_ary_finder.Not_found) break;	// no more </li>
			tmp_ary[tmp_idx++] = li_bgn;
			tmp_ary[tmp_idx++] = li_end + Li_end.length;
			pos = li_end + Li_end.length;
		}
		int ul_bgn_rhs = Byte_ary_finder.Find_fwd(src, Byte_ascii.Gt, ul_bgn);
		if (tmp_idx < 3	// 0 or 1 li; add everything between ul
			|| ul_bgn_rhs == Byte_ary_finder.Not_found
			) {
	...
	private static final byte[]
	  Ul_bgn = ByteAry_.new_utf8_("<ul")
	, Ul_end = ByteAry_.new_utf8_("</ul>")
	, Li_bgn = ByteAry_.new_utf8_("<li")
	, Li_end = ByteAry_.new_utf8_("</li>")
	;
}
