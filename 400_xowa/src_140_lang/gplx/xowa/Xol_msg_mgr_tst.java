...
class Xol_msg_mgr_fxt {
	public Xol_msg_mgr_fxt Clear() {
		...
		return this;
	}	Xoa_app app; Xow_wiki wiki; Xow_msg_mgr mgr;
	public void Test_val_by_key(String val, String expd) {
		Xol_msg_itm itm = wiki.Lang().Msg_mgr().Itm_by_key_or_new(ByteAry_.new_ascii_("test"));
		itm.Atrs_set(ByteAry_.new_ascii_(val), false, true);			
		Tfds.Eq(expd, String_.new_utf8_(wiki.Msg_mgr().Val_by_key_obj(ByteAry_.new_ascii_("test"))), "has_tmpl_txt");
	}
	public void Test_mediaWiki_msg(String raw, String expd) {
		byte[] msg_ttl = ByteAry_.new_ascii_("MediaWiki:msg_ttl");
		wiki.Db_mgr().Save_mgr().Data_create(Xoa_ttl.parse_(wiki, msg_ttl), ByteAry_.new_ascii_(raw));
		Tfds.Eq(expd, String_.new_utf8_(wiki.Msg_mgr().Val_by_key_obj(ByteAry_.new_ascii_("msg_ttl"))));
	}
