...
public class ByteTrieMgr_slim {
	public ByteTrieMgr_slim(boolean case_match) {root = new ByteTrieItm_slim(Byte_.Zero, null, !case_match);}	private ByteTrieItm_slim root;
	...
	public ByteTrieMgr_slim Add_byteVal_strAry(byte val, String... ary) {
		int ary_len = ary.length;
		ByteVal byteVal = ByteVal.new_(val);
		for (int i = 0; i < ary_len; i++) {
			String itm = ary[i];
			Add(ByteAry_.new_utf8_(itm), byteVal);
		}
		return this;
	}
	...
	public ByteTrieMgr_slim Add(String key, Object val) {return Add(ByteAry_.new_utf8_(key), val);}
