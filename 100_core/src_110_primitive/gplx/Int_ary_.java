...
public class Int_ary_ {
	public static int[] Parse_list_or(byte[] src, int[] or) {
		try {
		if (ByteAry_.Len_eq_0(src)) return or;	// null, "" should return [0]
		...
		while (true) {
			...
			if (	itm_done
				...
				int num = ByteAry_.XtoIntByPos(src, num_bgn, num_end, Int_.MinValue);
			...
			switch (b) {
				...
				case Byte_ascii.Dash:
					...
					rng_bgn = ByteAry_.XtoIntByPos(src, num_bgn, pos, Int_.MinValue);
