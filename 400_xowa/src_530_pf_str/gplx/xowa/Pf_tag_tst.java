...
public class Pf_tag_tst {
	...
	@Test  public void Basic()			{fxt.Test_parse_tmpl_str_test("{{#tag:div|a|atr1=val1|atr2=val2}}"					, "{{test}}"	, "<div atr1=\"val1\" atr2=\"val2\">a</div>");}
//		@Test  public void Missing_val()	{fxt.ini_Msg(Mwl_tag_rsc._.Invalid).Test_parse_tmpl_str_test("{{#tag:div|a|atr1=}}"	, "{{test}}"	, "");}	// see {{Reflist|colwidth=30em}} -> <ref group=a>a</ref>{{#tag:references||group=}} -> ""
	@Test  public void Atr2_empty()		{fxt.Test_parse_tmpl_str_test("{{#tag:div|a|atr1=val1|}}"							, "{{test}}"	, "<div atr1=\"val1\">a</div>");}	// see {{Reflist|colwidth=30em}} -> <ref group=a>a</ref>{{#tag:references||group=a|}} -> "<references group=a/>"
	@Test  public void Val_apos()		{fxt.Test_parse_tmpl_str_test("{{#tag:div|a|atr1='val1'}}"							, "{{test}}"	, "<div atr1=\"val1\">a</div>");}
	@Test  public void Val_quote()		{fxt.Test_parse_tmpl_str_test("{{#tag:div|a|atr1=\"val1\"}}"							, "{{test}}"	, "<div atr1=\"val1\">a</div>");}
	@Test  public void Ws_all()		    {fxt.Test_parse_tmpl_str_test("{{#tag:div|a|   atr1   =   val1   }}"					, "{{test}}"	, "<div atr1=\"val1\">a</div>");}
	@Test  public void Ws_quoted()		{fxt.Test_parse_tmpl_str_test("{{#tag:div|a|   atr1   = ' val1 ' }}"					, "{{test}}"	, "<div atr1=\" val1 \">a</div>");}
	@Test  public void Err_bad_key()	{fxt.Test_parse_tmpl_str_test("{{#tag:div|a|atr1=val|b}}"							, "{{test}}"	, "<div atr1=\"val\" \"b\">a</div>");}	// PURPOSE: b was failing b/c atr1 was larger and key_end set to 4 (whereas b was len=1)
...
//				, "{{test}}"	, "<div atr1=\" val1 \">a</div>");}
	@Test  public void Nested_tmpl() {	// PURPOSE: nested template must get re-evaluated; EX:de.wikipedia.org/wiki/Freiburg_im_Breisgau; DATE:2013-12-18
		...
		fxt.Test_parse_tmpl_str_test("{{#tag:span|{{A{{!}}expd}}}}"					, "{{test}}"	, "<span>pass</span>");
	}
}
