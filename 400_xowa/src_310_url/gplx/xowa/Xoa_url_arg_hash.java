...
public class Xoa_url_arg_hash {
	...
	public void Set_val_by_int(byte[] key, int val) {Set_val_by_bry(key, ByteAry_.new_ascii_(Int_.XtoStr(val)));}
	public void Set_val_by_bry(byte[] key, byte[] val) {		
		Gfo_url_arg arg = (Gfo_url_arg)hash.Fetch(key);
		if (arg == null) {
			arg = new Gfo_url_arg(key, ByteAry_.Empty);
			hash.Add(key, arg);
		}
		arg.Val_bry_(val);
	}
	public byte[] Concat(ByteAryBfr bfr, byte[]... ary) {
	...
	public static void Concat_bfr(ByteAryBfr bfr, Gfo_url_arg[] ary) {
		int ary_len = ary.length;
		for (int i = 0; i < ary_len; i++) {
			Gfo_url_arg itm = ary[i];
