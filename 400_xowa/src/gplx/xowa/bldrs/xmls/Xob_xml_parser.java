			...
			if (o == null) {								// text_data; not an xml_nde (<id>), xml_escape (&lt;), or tab
				...
				switch (itm.Tid()) {
					...
					case Xob_xml_parser_.Id_id_end:	
						if (page_id_needed) {
							int page_id = ByteAry_.XtoIntByPos(src, data_bgn, hook_bgn, -1); if (page_id == -1) usr_dlg.Warn_many(GRP_KEY, "page_id_invalid", "page_id_is_invalid: ~{0}", String_.new_utf8_(src, data_bgn, hook_bgn));
							rv.Id_(page_id);
