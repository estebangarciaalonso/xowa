...
public class Xow_xwiki_mgr implements GfoInvkAble {
	...
	public void Add_full(String alias, String domain) {Add_full(ByteAry_.new_ascii_(alias), ByteAry_.new_ascii_(domain), null);}
	...
	public void Add_full(byte[] alias, byte[] domain, byte[] fmt) {
		...
		int lang_tid = -1;
		Xow_wiki_type wiki_type = Xow_wiki_type_.parse_(domain);
	...
	public void Add_bulk(byte[] raw) {
		byte[][] rows = ByteAry_.Split(raw, Byte_ascii.NewLine);
		int rows_len = rows.length;
		Hash_adp_bry lang_regy = Xol_lang_itm_.Regy();
		for (int i = 0; i < rows_len; i++) {
			byte[] row = rows[i]; if (ByteAry_.Len_eq_0(row)) continue;	// ignore blank rows
	...
	public Xow_xwiki_itm Add_bulk_row(Hash_adp_bry lang_regy, byte[] row) {
		...
		Xow_wiki_type wiki_type = Xow_wiki_type_.parse_(wiki_type_bry);
		int lang_tid = Xol_lang_itm_.Id__null;
	...
	public void Add_bulk_peers(byte[] raw) {
		byte[][] keys = ByteAry_.Split(raw, Byte_ascii.Tilde);
		int len = keys.length;
		...
		if (lang_key_bry == Xol_lang_itm_.Bry__null) lang_key_bry = Xol_lang_.Key_en;	// default non-lang wikis to english
		...
		for (int i = 0; i < len; i++) {
			...
			byte xwiki_tid_val = Xow_wiki_type_.Tid_by_name(wiki_name_bry);
	...
	public void Add_bulk_langs(GfoMsg m) {
		...
		byte wiki_tid = wiki_type_name == null ? wiki.Wiki_tid() : Xow_wiki_type_.Tid_by_name(wiki_type_name);
	...
	void Add_bulk_langs(byte[] grp_key, byte wiki_tid) {
		...
		for (int i = 0; i < len; i++) {
			...
			Xow_xwiki_itm xwiki = new Xow_xwiki_itm(lang.Key_bry(), ByteAry_.new_utf8_(fmt), wiki_tid, lang_id, domain_bry).Offline_(offline_exists);
			Add_itm(xwiki, lang);
		}
		lang_mgr.Grps_sort();
