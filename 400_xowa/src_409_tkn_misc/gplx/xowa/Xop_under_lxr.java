...
class Xop_under_lxr implements Xop_lxr {
	...
	public int Make_tkn(Xop_ctx ctx, Xop_tkn_mkr tkn_mkr, Xop_root_tkn root, byte[] src, int src_len, int bgn_pos, int cur_pos) {
		...
		if (o != null) {
			...
			switch (id_val.Val()) {
				case Xol_kwd_grp_.Id_toc:				hdr_mgr.Toc_manual_(); ctx.Subs_add(root, tkn_mkr.Under(bgn_pos, cur_pos, id_val.Val())); break;	// NOTE: only save under_tkn for TOC (b/c its position is needed for insertion); DATE:2013-07-01
