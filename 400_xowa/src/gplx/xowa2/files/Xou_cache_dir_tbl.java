...
import gplx.dbs.*; import gplx.dbs.qrys.*; import gplx.xowa.files.*;
class Xou_cache_dir_tbl {
	...
	public String Commit(Xou_cache_dir itm) {
		try {
			...
			switch (itm.Cmd_mode()) {
				case Db_cmd_mode.Tid_create:	stmt.Clear().Val_int(itm.Dir_id()); Commit_stmt_args(stmt, itm); stmt.Exec_insert(); break;
				case Db_cmd_mode.Tid_update:	stmt.Clear();						 Commit_stmt_args(stmt, itm); stmt.Val_int(itm.Dir_id()).Exec_update(); break;
				case Db_cmd_mode.Tid_delete:	stmt.Clear().Val_int(itm.Dir_id()); stmt.Exec_delete();	break;
