...
interface Url_encoder_itm {
	int Encode(ByteAryBfr bfr, byte[] src, int end, int idx, byte b);
	int Decode(ByteAryBfr bfr, byte[] src, int end, int idx, byte b, boolean fail_when_invalid);
}
class Url_encoder_itm_same implements Url_encoder_itm {
	public int Encode(ByteAryBfr bfr, byte[] src, int end, int idx, byte b) {bfr.Add_byte(b); return 0;}
	public int Decode(ByteAryBfr bfr, byte[] src, int end, int idx, byte b, boolean fail_when_invalid) {bfr.Add_byte(b); return 0;}
...
class Url_encoder_itm_diff implements Url_encoder_itm {
	...
	public int Encode(ByteAryBfr bfr, byte[] src, int end, int idx, byte b) {bfr.Add_byte(repl); return 0;}
	public int Decode(ByteAryBfr bfr, byte[] src, int end, int idx, byte b, boolean fail_when_invalid) {bfr.Add_byte(orig); return 0;}
} 
class Url_encoder_itm_hex implements Url_encoder_itm {
	...
	public int Encode(ByteAryBfr bfr, byte[] src, int end, int idx, byte b) {Encode_byte(b, bfr, encode_marker); return 0;}
	public static void Encode_byte(byte b, ByteAryBfr bfr, byte encode_marker) {
	...
	public int Decode(ByteAryBfr bfr, byte[] src, int end, int idx, byte b, boolean fail_when_invalid) {
		if (idx + 2 >= end) {
			if (fail_when_invalid) throw Err_.new_fmt_("decode needs 3 bytes: idx={0} len={1} snip={2}", idx, end, String_.new_utf8_(ByteAry_.Mid_by_len_safe(src, idx, 3)));
			else {
				bfr.Add_byte(b);
				return 0;
			}
		...
		if (fail_when_invalid)
			throw Err_.new_fmt_("decode is invalid: idx={0}, snip={1}", idx, String_.new_utf8_(ByteAry_.Mid_by_len_safe(src, idx, 3)));
		else {
			bfr.Add_byte(b);
			return 0;
		}
...
class Url_encoder_itm_html_ent implements Url_encoder_itm {
	...
	public int Encode(ByteAryBfr bfr, byte[] src, int end, int idx, byte b) {
		...
		if (o == null) {	// unknown entity (EX:&unknown;); return &;
			...
			gplx.xowa.Xop_amp_trie_itm itm = (gplx.xowa.Xop_amp_trie_itm)o;
