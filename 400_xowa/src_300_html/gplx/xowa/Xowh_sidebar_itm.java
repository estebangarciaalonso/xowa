...
public class Xowh_sidebar_itm {
	...
	public byte[] Title() {return title;} public Xowh_sidebar_itm Title_(byte[] v) {title = v; return this;} private byte[] title = ByteAry_.Empty;
	...
	public byte[] Atr_accesskey_and_title() {return atr_accesskey_and_title;} public Xowh_sidebar_itm Atr_accesskey_and_title_(byte[] v) {atr_accesskey_and_title = v; return this;} private byte[] atr_accesskey_and_title = ByteAry_.Empty;
...
class Xowh_sidebar_grp_fmtr_arg implements ByteAryFmtrArg {
	public void Grp_(Xowh_sidebar_itm grp, ByteAryFmtr fmtr) {this.grp = grp; this.fmtr = fmtr;} private Xowh_sidebar_itm grp; ByteAryFmtr fmtr;
	public void XferAry(ByteAryBfr bfr, int idx) {
		int len = grp.Itms_len();
		for (int i = 0; i < len; i++) {
			Xowh_sidebar_itm itm = (Xowh_sidebar_itm )grp.Itms_get_at(i);
			fmtr.Bld_bfr_many(bfr, itm.Id(), itm.Href(), itm.Atr_accesskey_and_title(), itm.Text());
		}		
	}
}
